 ...
 *
 * <p>Provides a unified API for loading YAML configuration files from either:
 *
 * <ol>
 *   <li>Filesystem (absolute or relative paths)
 *   <li>Classpath resources (fallback)
 * </ol>
 *
 * <p><strong>Loading Strategy:</strong>
 *
 * <ol>
 *   <li>Check if file exists at resolved filesystem path
 *   <li>If exists, load from filesystem
 *   <li>Otherwise, attempt to load from classpath
 *   <li>Throw {@link ConfigurationException} if not found in either location
 * </ol>
 *
 * <p><strong>Path Resolution:</strong>
 *
 * <p>Resolves paths relative to a base directory with security validation:
 *
 * <ul>
 *   <li>Normalizes paths to prevent directory traversal
 *   <li>Validates resolved path is within base directory
 *   <li>Throws exception for paths outside base directory
 * </ul>
 *
 * <p><strong>Jackson Integration:</strong>
 *
 * <p>Uses Jackson's YAMLFactory for parsing. Supports:
 *
 * <ul>
 *   <li><strong>Class deserialization:</strong> {@code load(path, MyConfig.class)}
 *   <li><strong>Generic types:</strong> {@code load(path, new TypeReference<Map<String, Object>>()
 *       {})}
 * </ul>
 *
 ...
 * }</pre>
 *
 * <p><strong>ConfigSource Integration:</strong>
 *
 * <p>Also supports {@link ConfigSource} abstraction for custom configuration sources.
 *
 * <p><strong>TODO:</strong> Review config path handling and complete class implementation.
 *
 * @see ConfigSource
...
public class ConfigLoader {
   ...
   *
   * @param baseDir Base folder provided by the implementation
   * @param resourcePath Path relative to the baseDir
   * @param configClass Class to deserialize
   */
  ...
  private Path resolvePath(Path baseDir, String resourcePath) {
    Path path =
        baseDir
            .resolve(resourcePath.startsWith("/") ? resourcePath.substring(1) : resourcePath)
            .toAbsolutePath()
            .normalize();
