 ...
 *
 * <p>This package provides robust configuration loading with support for:
 *
 * <ul>
 *   <li><strong>ConfigLoader</strong> - YAML file loader with Jackson integration
 *   <li><strong>ConfigSource</strong> - Sealed abstraction for classpath/filesystem sources
 *   <li><strong>ConfigurationException</strong> - Configuration-specific exceptions
 * </ul>
 *
 * <p><strong>Loading Strategies:</strong>
 *
 * <p>The {@link io.cheshire.common.config.ConfigSource} sealed interface supports two loading
 * strategies:
 *
 * <pre>
 * ConfigSource (sealed)
 ...
 * Map<String, Object> config = loader.load(source, "dynamic.yaml", new TypeReference<Map<String, Object>>() {
 * });
 * }</pre>
 *
 * <p><strong>Security:</strong>
 *
 * <p>The filesystem loader prevents directory traversal attacks:
 *
 * <pre>{@code
 ...
 *
 * <p>ConfigLoader attempts filesystem first, then falls back to classpath:
 *
 * <ol>
 *   <li>Check if file exists on filesystem relative to base directory
 *   <li>If found, load from filesystem
 *   <li>If not found, attempt to load from classpath
 *   <li>If still not found, throw ConfigurationException
 * </ol>
 *
 ...
 * <ul>
 *   <li>YAML (.yaml, .yml)
 *   <li>JSON (.json) - via Jackson YAML parser
 * </ul>
 *
 * <p><strong>Error Handling:</strong>
 *
 * <p>All configuration errors are wrapped in {@link
 * io.cheshire.common.config.ConfigurationException}:
 *
 * <pre>{@code
 * try {
 ...
 *
 * <p>ConfigLoader supports both class-based and generic type loading:
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p><strong>Integration with CheshireBootstrap:</strong>
 *
 * <pre>{@code
 * // Bootstrap uses ConfigLoader internally
 * CheshireSession session = CheshireBootstrap.fromClasspath("config") // Uses ConfigSource.classpath()
 *         .build();
 *
 * // Or from filesystem
 * CheshireSession session = CheshireBootstrap.fromFilesystem(Path.of("/etc/cheshire")) // Uses
