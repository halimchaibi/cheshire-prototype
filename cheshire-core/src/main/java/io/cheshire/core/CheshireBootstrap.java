 ...
 * CheshireSession session = CheshireBootstrap
 *     .fromDirectory(Path.of("/etc/cheshire"))
 *     .skipSessionAutoStart()  // Manual start control
 *     .build();
 *
 ...
 * Bootstrap failures throw {@link CheshireException} with detailed context about
 * which phase failed (config loading, lifecycle init, or session building).
 *
 * @see CheshireSession
...
//    }
     ...
     * <p>
     * This is the primary entry point for embedded applications, tests, and JAR deployments
     * where configuration is packaged within the application.
     * <p>
    ...
    public static CheshireBootstrap fromClasspath(String resourceRoot) {
        return new CheshireBootstrap(
                new ConfigurationManager(ConfigSource.classpath(resourceRoot)),
                true
        );
    }
     ...
     * <p>
     * This is the primary entry point for production deployments where configuration
     * is externalized and managed separately from the application binary.
     * <p>
     ...
     * </ul>
     * <p>
     * <strong>Path traversal security:</strong> Configuration loading enforces that all
     * referenced files remain within the specified base directory to prevent path
     * traversal attacks.
     *
    ...
    public static CheshireBootstrap fromDirectory(Path dir) {
        return new CheshireBootstrap(
                new ConfigurationManager(ConfigSource.filesystem(dir)),
                true
        );
    }
     ...
     * <p>
     * By default, {@link #build()} calls {@link CheshireSession#start()} automatically.
     * Use this method when you need manual control over session lifecycle, such as:
     * <ul>
     ...
     * <p>
     * Useful for advanced scenarios requiring direct access to configuration
     * details before session construction.
     *
     ...
     * </pre>
     * <p>
     * The session includes shutdown hooks for graceful cleanup of all initialized
     * components in reverse order.
     *
     * @return fully initialized and optionally started CheshireSession
     * @throws CheshireException if configuration loading, lifecycle initialization, or session build fails
     */
    public CheshireSession build() {
        try {
            ...
            var session = CheshireSession.builder()
            ...
            throw new CheshireException("Failed to bootstrap Cheshire Core - error during configuration loading, lifecycle initialization, or session building", e);
        }
    }
