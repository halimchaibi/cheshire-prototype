 ...
 *
 * <p>Represents a <b>long-lived connection context</b> (e.g., an MCP session, WebSocket, or
 * persistent SSE stream). Unlike {@link io.cheshire.core.server.protocol.RequestContext}, which is
 * ephemeral and scoped to a single request-response cycle, SessionContext persists across multiple
 * distinct actions within the same session.
 *
 * <h3>Core Responsibilities</h3>
 *
 * <ul>
 *   <li><b>Identity:</b> Tracks {@code sessionId} and {@code userId} for the lifetime of the
 *       connection.
 *   <li><b>Observability:</b> Maintains {@code traceId} for distributed tracing across session
 *       operations.
 *   <li><b>Governance:</b> Holds a {@code securityContext} for session-level Role-Based Access
 *       Control (RBAC).
 *   <li><b>Lifecycle Tracking:</b> Records {@code createdAt} (session start) and {@code
 *       arrivalTime} (current pulse).
 * </ul>
 *
 * <h3>Mutability Model</h3>
 *
 * <p>This record follows a <b>fully immutable</b> design pattern:
 *
 * <ul>
 *   <li><b>Immutable Maps:</b> Both {@code securityContext} and {@code attributes} are exposed as
 *       unmodifiable maps to prevent accidental modification.
 *   <li><b>Functional Updates:</b> Use {@link #mutateAttribute(String, Object)} to create a new
 *       SessionContext instance with modified attributes (copy-on-write pattern).
 *   <li><b>Thread Safety:</b> Immutability guarantees thread-safe reads without synchronization.
 * </ul>
 *
 * <h3>Usage Example</h3>
 *
...
public record SessionContext(
   ...
   *
   * <p>Creates a SessionContext with:
   *
