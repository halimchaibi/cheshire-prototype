 ...
 * <h1>SessionTask</h1>
 *
 * <p>The <b>Execution Context</b> for internal business logic. This record serves as a
 * specialized data carrier that simplifies how developers interact with raw inputs
 * and operational metadata during the execution phase of an action.</p>
 *
 *
 *
 * <h3>Key Design Patterns</h3>
 * <ul>
 * <li><b>Immutable Transformations:</b> Uses the "Wither" pattern via {@link #withMetadata(UnaryOperator)}
 * to return new instances rather than mutating state, ensuring safety in reactive pipelines.</li>
 * <li><b>Defensive Type Safety:</b> Provides "Safe Cast" utilities ({@code contextAs}, {@code valueAs})
 * to eliminate {@code ClassCastException} risks during runtime.</li>
 * <li><b>Required Input Enforcement:</b> Includes a "fail-fast" mechanism via {@link #require(String)}
 * for mandatory parameters.</li>
 * </ul>
 *
...
public record SessionTask(java.util.Map<String, Object> data, java.util.Map<String, Object> metadata) {
     ...
     * <p>Use this for mandatory inputs where the absence of data prevents execution.</p>
     *
    ...
    /**
     * Extracts a value from the data map with strict type validation,
     * throwing a descriptive exception if the key is missing or the type is mismatched.
     * * <p>This ensures "fail-fast" behavior in the pipeline: if a Pre-Processor
     * stashed the wrong type, the Executor will catch it immediately.</p>
     *
    ...
    public <T> T requireAs(String key, Class<T> type) {
        Object value = java.util.Optional.ofNullable(data.get(key))
    ...
    public <T> T requireMetaAs(String key, Class<T> type) {
        Object value = java.util.Optional.ofNullable(metadata.get(key))
                .orElseThrow(() -> new IllegalArgumentException(
                        "Execution Boundary Violation: Missing required attribute '%s'".formatted(key)
                ));
     ...
     * <p>This follows the immutable update pattern, preserving the original task state.</p>
     *
     ...
     * <p>Useful for generic lookups where a value might be either a parameter or an attribute.</p>
     *
     ...
     * <p>If a specific "payload" key exists in the data map, it is cast;
     * otherwise, the entire data map is cast to the target type.</p>
     *
