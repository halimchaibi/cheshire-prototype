 ...
 *
 * <p>
 * The <b>Execution Context</b> for internal business logic. This record serves as a specialized data carrier that
 * simplifies how developers interact with raw inputs and operational metadata during the execution phase of an action.
 * </p>
 *
 *
 *
 ...
 * <ul>
 * <li><b>Immutable Transformations:</b> Uses the "Wither" pattern via {@link #withMetadata(UnaryOperator)} to return
 * new instances rather than mutating state, ensuring safety in reactive pipelines.</li>
 * <li><b>Defensive Type Safety:</b> Provides "Safe Cast" utilities ({@code contextAs}, {@code valueAs}) to eliminate
 * {@code ClassCastException} risks during runtime.</li>
 * <li><b>Required Input Enforcement:</b> Includes a "fail-fast" mechanism via {@link #require(String)} for mandatory
 * parameters.</li>
 * </ul>
 *
 ...
 *
 * @param data
 *            The primary domain data (e.g., arguments for a tool call).
 * @param metadata
 *            Contextual operational data (e.g., security scopes, session IDs).
...
public record SessionTask(java.util.Map<String, Object> data, java.util.Map<String, Object> metadata) {
     ...
     * <p>
     * Use this for mandatory inputs where the absence of data prevents execution.
     * </p>
     *
     * @param key
     ...
     *             if the key is not present in the data map.
     */
    @SuppressWarnings("unchecked")
    public <T> T require(String key) {
        return (T) java.util.Optional.ofNullable(data.get(key))
                .orElseThrow(() -> new IllegalArgumentException("Missing required input: " + key));
    }

    /**
     * Extracts a value from the data map with strict type validation, throwing a descriptive exception if the key is
     * missing or the type is mismatched. *
     * <p>
     * This ensures "fail-fast" behavior in the pipeline: if a Pre-Processor stashed the wrong type, the Executor will
     * catch it immediately.
     * </p>
     *
     * @param key
     *            The attribute identifier to retrieve.
     * @param type
     *            The class representing the expected type {@code T}.
     * @param <T>
     ...
     *             if the value exists but is not assignable to {@code type}.
     */
    public <T> T requireAs(String key, Class<T> type) {
        Object value = java.util.Optional.ofNullable(data.get(key)).orElseThrow(() -> new IllegalArgumentException(
                "Execution Boundary Violation: Missing required attribute '%s'".formatted(key)));

        if (!type.isInstance(value)) {
            throw new ClassCastException("Attribute Type Mismatch: Key '%s' expected %s but found %s".formatted(key,
    ...
    public <T> T requireMetaAs(String key, Class<T> type) {
        Object value = java.util.Optional.ofNullable(metadata.get(key)).orElseThrow(() -> new IllegalArgumentException(
                "Execution Boundary Violation: Missing required attribute '%s'".formatted(key)));
     ...
     * <p>
     * This follows the immutable update pattern, preserving the original task state.
     * </p>
     *
     * @param transform
     *            A function that accepts the current metadata and returns an updated map.
     * @return A new SessionTask with the updated metadata.
     */
    public SessionTask withMetadata(java.util.function.UnaryOperator<java.util.Map<String, Object>> transform) {
     ...
     *            The metadata key.
     * @param clazz
     *            The target class for type checking.
     * @param <T>
     *            The expected type.
     ...
     * <p>
     * Useful for generic lookups where a value might be either a parameter or an attribute.
     * </p>
     *
     * @param key
     *            The search key.
     ...
     * <p>
     * If a specific "payload" key exists in the data map, it is cast; otherwise, the entire data map is cast to the
     * target type.
     * </p>
     *
     * @param clazz
