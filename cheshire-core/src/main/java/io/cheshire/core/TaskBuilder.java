 ...
 *
 * <p>
 * A <b>Domain Mapper</b> that transforms a protocol-level {@link RequestEnvelope} into an execution-ready
 * {@link SessionTask}. This builder handles the "flattening" of disparate data sources—routing info, payload arguments,
 * and session context—into a unified task structure.
 * </p>
 *
 *
 *
 ...
 * <ul>
 * <li><b>Argument Extraction:</b> Merges the primary {@code payload.data()} (under the "payload" key) with the
 * flattened {@code payload.parameters()} map.</li>
 * <li><b>Context Extraction:</b> Captures essential traceability (requestId, receivedAt) and routing (capability,
 * action) metadata, merging them with the mutable {@link RequestContext} attributes.</li>
 * </ul>
 *
 * <h3>Design Invariants</h3>
 * <ul>
 * <li><b>Immutability:</b> The resulting {@code SessionTask} is populated with unmodifiable maps via
 * {@link java.util.Map#copyOf(Map)}, ensuring that the task state remains stable during execution.</li>
...
public class TaskBuilder {
    ...
    /**
     * Entry point for the builder. * @param envelope The protocol-level request container.
     *
     ...
     * <p>
     * This method executes the internal mapping logic for arguments and context.
     * </p>
     *
     * @return A fully populated, immutable SessionTask.
     */
    public SessionTask build() {
        return new SessionTask(extractData(), extractMetadata());
    }
    ...
    private java.util.Map<String, Object> extractMetadata() {
        ...
        if (envelope.context() != null) {
            ...
            if (context.userId() != null)
                meta.put(Key.USER_ID.key(), context.userId());
