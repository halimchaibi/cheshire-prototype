 ...
 * <h1>TaskBuilder</h1>
 *
 * <p>A <b>Domain Mapper</b> that transforms a protocol-level {@link RequestEnvelope} into an
 * execution-ready {@link SessionTask}. This builder handles the "flattening" of disparate data
 * sources—routing info, payload arguments, and session context—into a unified task structure.
 *
 * <h3>The Extraction Strategy</h3>
 *
 * <ul>
 *   <li><b>Argument Extraction:</b> Merges the primary {@code payload.data()} (under the "payload"
 *       key) with the flattened {@code payload.parameters()} map.
 *   <li><b>Context Extraction:</b> Captures essential traceability (requestId, receivedAt) and
 *       routing (capability, action) metadata, merging them with the mutable {@link RequestContext}
 *       attributes.
 * </ul>
 *
 ...
 *       via {@link java.util.Map#copyOf(Map)}, ensuring that the task state remains stable during
 *       execution.
 *   <li><b>Null Safety:</b> Gracefully handles null payloads or contexts by initializing empty
 *       maps.
 * </ul>
 *
 * <pre>{@code
...
public class TaskBuilder {
   ...
   *
   * <p>This method executes the internal mapping logic for arguments and context.
   *
