 ...
 * <h1>TaskResult</h1>
 *
 * <p>A <b>Sealed Algebraic Data Type (ADT)</b> representing the final outcome of a {@code
 * CheshireSession} execution. It encapsulates either a successful pipeline output or a categorized
 * failure.
 *
 * <h3>The Success Record</h3>
 *
 * <p>The {@link Success} variant carries the primary domain {@code output}. It provides Scala-like
 * convenience methods for type-safe casting and metadata extraction, mirroring the API style found
 * in {@code SessionTask}.
 *
 * <h3>The Failure Record</h3>
 *
 * <p>The {@link Failure} variant captures why a task could not be completed. It bridges the
 * internal pipeline exceptions with the standardized {@link Status} codes, allowing the
 * orchestrator to decide how to report the error to the end user.
 *
 * <h3>Design Invariants</h3>
 *
 * <ul>
 *   <li><b>Immutability:</b> Both records use {@link java.util.Map#copyOf(Map)} in their canonical
 *       constructors to ensure that metadata cannot be mutated after the task completes.
 *   <li><b>Exhaustiveness:</b> Being {@code sealed}, this interface allows developers to use Java
 *       21+ switch expressions to handle results without a {@code default} clause.
 * </ul>
 *
 * <pre>{@code
...
public sealed interface TaskResult permits TaskResult.Failure, TaskResult.Success {
   ...
   *
   * @param output The primary data produced by the pipeline.
   * @param metadata Operational metadata (e.g., execution time, trace IDs).
   */
  record Success(Object output, java.util.Map<String, Object> metadata) implements TaskResult {
    public Success {
      metadata =
          java.util.Map.copyOf(
              metadata != null ? metadata : java.util.Map.of("empty-metadata", true));
    }
     ...
     * @throws ClassCastException if the output is not compatible.
     */
    ...
    /**
     * "Fail-fast" metadata extraction. Use this for mandatory metadata like 'traceId' or
     * 'tenantId'.
     */
    public <T> T requireMeta(String key, Class<T> clazz) {
      return metaAs(key, clazz)
          .orElseThrow(
              () ->
                  new IllegalArgumentException(
                      String.format(
                          "Missing or invalid metadata: '%s' (expected %s)",
   ...
   *
   * @param status The logical category of the error (e.g., BAD_REQUEST).
   * @param cause The underlying exception that triggered the failure.
   * @param metadata Optional context about the failure state.
   */
  record Failure(Status status, Throwable cause, java.util.Map<String, Object> metadata)
     ...
     *
     * @param status The failure category.
