 ...
 * <h1>TaskResult</h1>
 *
 * <p>A <b>Sealed Algebraic Data Type (ADT)</b> representing the final outcome of a
 * {@code CheshireSession} execution. It encapsulates either a successful pipeline
 * output or a categorized failure.</p>
 *
 *
 *
 * <h3>The Success Record</h3>
 * <p>The {@link Success} variant carries the primary domain {@code output}. It provides
 * Scala-like convenience methods for type-safe casting and metadata extraction,
 * mirroring the API style found in {@code SessionTask}.</p>
 *
 * <h3>The Failure Record</h3>
 * <p>The {@link Failure} variant captures why a task could not be completed. It bridges
 * the internal pipeline exceptions with the standardized {@link Status} codes,
 * allowing the orchestrator to decide how to report the error to the end user.</p>
 *
 * <h3>Design Invariants</h3>
 * <ul>
 * <li><b>Immutability:</b> Both records use {@link java.util.Map#copyOf(Map)} in their
 * canonical constructors to ensure that metadata cannot be mutated after the task completes.</li>
 * <li><b>Exhaustiveness:</b> Being {@code sealed}, this interface allows developers to
 * use Java 21+ switch expressions to handle results without a {@code default} clause.</li>
 * </ul>
 *
 * <pre>{@code
 ...
 *
 * switch (result) {
 * case TaskResult.Success(var out, var meta) ->
 * log.info("Task completed: {}", out);
 * case TaskResult.Failure(var status, var cause, var meta) ->
...
public sealed interface TaskResult permits TaskResult.Failure, TaskResult.Success {
    ...
    record Success(Object output, java.util.Map<String, Object> metadata) implements TaskResult {
        ...
        /**
         * "Fail-fast" metadata extraction.
         * Use this for mandatory metadata like 'traceId' or 'tenantId'.
         */
