 ...
 * </ul>
 * <p>
 * <strong>Architecture:</strong> Each capability is independently deployable and can be exposed through multiple
 * protocols simultaneously. The framework routes requests to the appropriate capability based on the request context.
 * <p>
 * <strong>Immutability:</strong> This record is immutable by design, ensuring thread-safety and preventing accidental
 * modifications during runtime.
 *
 * @param name
 *            unique identifier for this capability (e.g., "blogrest", "authors")
 * @param description
 *            human-readable description of the capability's purpose
 * @param domain
 *            business domain this capability belongs to (e.g., "blog", "ecommerce")
 * @param exposure
 *            configuration for how this capability is exposed (REST, MCP)
 * @param transport
 *            network transport configuration (HTTP, stdio)
 * @param sources
 *            list of data source names this capability uses
 * @param engine
 *            query engine name used for data operations
 * @param pipelines
 *            map of action names to their processing pipelines
 * @param actions
 *            configuration of all actions (tools/endpoints) in this capability
