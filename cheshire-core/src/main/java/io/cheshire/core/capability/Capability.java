 ...
 *
 * <p>A capability is a cohesive business domain that federates:
 *
 * <ul>
 *   <li><strong>Data Sources:</strong> Databases, APIs, file systems
 *   <li><strong>Actions:</strong> Operations (tools/endpoints) exposed to clients
 *   <li><strong>Pipelines:</strong> Processing logic (preprocess → execute → postprocess)
 *   <li><strong>Exposure:</strong> How it's accessed (REST API, MCP stdio, MCP HTTP)
 *   <li><strong>Transport:</strong> Network protocol (HTTP, stdio)
 * </ul>
 *
 * <p><strong>Examples:</strong>
 *
 * <ul>
 *   <li><strong>Authors:</strong> Manages author CRUD operations
 *   <li><strong>Articles:</strong> Handles article publishing, search, and retrieval
 *   <li><strong>Comments:</strong> Manages article comments and moderation
 * </ul>
 *
 * <p><strong>Architecture:</strong> Each capability is independently deployable and can be exposed
 * through multiple protocols simultaneously. The framework routes requests to the appropriate
 * capability based on the request context.
 *
 * <p><strong>Immutability:</strong> This record is immutable by design, ensuring thread-safety and
