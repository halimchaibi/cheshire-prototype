 ...
 * <p>
 * ConfigurationManager is responsible for loading, resolving, validating, and safely exposing Cheshire configuration
 * from various sources (filesystem, classpath).
 * <p>
 * <strong>Responsibilities:</strong>
 * <ul>
 * <li>Load main {@link CheshireConfig} from YAML files</li>
 * <li>Resolve capability references (actions, pipelines)</li>
 * <li>Validate configuration structure and references</li>
 * <li>Provide defensive copies to prevent external modification</li>
 * </ul>
 * <p>
 * <strong>Configuration Loading Process:</strong>
 * <ol>
 * <li>Load main config file (cheshire.yaml or from system property)</li>
 * <li>Resolve capability-specific files (actions-specification-file, pipelines-definition-file)</li>
 * <li>Validate all required fields and cross-references</li>
 * <li>Store validated configuration for session initialization</li>
 * </ol>
 * <p>
 ...
 * </pre>
 * <p>
 * <strong>File Resolution:</strong> Main config file determined by (in order):
 * <ol>
 * <li>System property: {@code cheshire.config}</li>
 * <li>Environment variable: {@code CHESHIRE_CONFIG}</li>
 * <li>Default: {@code cheshire.yaml}</li>
 * </ol>
 * <p>
 * Capability-specific files loaded relative to the main config source.
 * <p>
 * <strong>Validation:</strong>
 * <ul>
 * <li>Required fields: name, exposure, transport, actions-specification-file</li>
 * <li>Source references must exist in sources configuration</li>
 * <li>Exposures must be defined</li>
 * </ul>
 * <p>
 * <strong>Thread Safety:</strong> Immutable after initialization. {@link #getCheshireConfig()} returns defensive
 * copies.
 *
 * @see CheshireConfig
 * @see ConfigLoader
...
public final class ConfigurationManager {
     ...
     * <p>
     * This is the primary constructor used by {@link io.cheshire.core.CheshireBootstrap}.
     * <p>
     * <strong>Initialization sequence:</strong>
     * <ol>
     * <li>Determine config filename from system property/env/default</li>
     * <li>Load main {@link CheshireConfig} via {@link ConfigLoader}</li>
     * <li>Resolve capability references (actions, pipelines)</li>
     * <li>Validate complete configuration</li>
     * </ol>
     *
     * @param source
     *            the configuration source (classpath or filesystem)
     * @throws ConfigurationException
     *             if loading or validation fails
     */
    public ConfigurationManager(ConfigSource source) {
        String configFile = System.getProperty("cheshire.config",
                System.getenv().getOrDefault("CHESHIRE_CONFIG", "cheshire.yaml"));
        this.source = source;
     ...
     * <p>
     * This constructor is used for testing and programmatic configuration where YAML file loading is bypassed.
     * <p>
     * <strong>Note:</strong> No filesystem/classpath access. Capability references (actions, pipelines) must be
     * pre-populated in the config object.
     *
     * @param cheshireConfig
     *            fully configured CheshireConfig instance
     * @throws ConfigurationException
     *             if config is null or validation fails
     */
     ...
     * @param id
     *            the capability ID for error context
     * @param errors
     ...
     * <p>
     * Called automatically by constructors. Not intended for external use.
     */
     ...
     * <p>
     * <strong>Why defensive copy?</strong> Prevents external modification of internal configuration state. Required to
     * avoid SpotBugs warning EI_EXPOSE_REP (Exposing Internal Representation).
     * <p>
     * <strong>Performance Note:</strong> This operation uses deep cloning via JSON serialization/deserialization. Cache
     ...
     * <p>
     * For each capability, loads:
     * <ul>
     * <li>Actions specification (MCP tools, REST endpoints)</li>
     * <li>Pipeline definitions (PreProcessor, Executor, PostProcessor configs)</li>
     * </ul>
     * <p>
     * Files are loaded relative to the main configuration source.
     */
    private void resolveReferences() {
        ...
        cheshireConfig.getCapabilities().values().forEach(capability -> {
            capability.setActions(loadActionsSpecification(capability.getActionsSpecificationFile()));

            capability.setPipelines(loadPipelinesDefinition(capability.getPipelinesDefinitionFile()));
        });
    }
     ...
     * <p>
     * Pipelines define the three-stage processing for each action: PreProcessor → Executor → PostProcessor
     * <p>
     * Each pipeline specifies:
     * <ul>
     * <li>Implementation classes</li>
     * <li>DSL query templates</li>
     * <li>Validation rules</li>
     * </ul>
     *
     * @param path
     *            relative path to pipelines YAML file
     ...
     * <p>
     * Actions define the operations exposed by the capability:
     * <ul>
     * <li>MCP: tools with input schemas</li>
     * <li>REST: endpoints with HTTP methods</li>
     * <li>GraphQL: queries and mutations</li>
     * </ul>
     *
     * @param path
     *            relative path to actions YAML file
     ...
     * <ul>
     * <li>Required capability fields (name, exposure, transport, etc.)</li>
     * <li>Source references exist</li>
     * <li>Exposures are defined</li>
     * <li>No dangling references</li>
     * </ul>
     * <p>
     * Accumulates all errors before throwing to provide complete feedback.
     *
     * @throws ConfigurationException
     ...
     * <p>
     * Checks each capability for:
     * <ul>
     * <li>Required fields present</li>
     * <li>Source references valid</li>
     * <li>Actions and pipelines loadable</li>
     * </ul>
     *
     * @param errors
     *            list to accumulate validation errors
     ...
     * <p>
     * Ensures at least one exposure is defined for the framework to be operational.
     *
     * @param errors
     *            list to accumulate validation errors
     */
