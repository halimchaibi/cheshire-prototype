 ...
 * <p>
 * LifecycleManager orchestrates the startup and teardown of Cheshire components
 * in the correct order, ensuring dependencies are satisfied and resources are
 * properly cleaned up.
 * <p>
 ...
 * </ol>
 * <p>
 * <strong>Parallel Initialization:</strong>
 * Components within the same phase initialize concurrently using
 * {@link CompletableFuture} for faster startup. Uses a cached thread pool
 * to handle variable numbers of components.
 * <p>
 * <strong>Shutdown Sequence:</strong>
 * Components shut down in <strong>reverse order</strong> (LIFO) to ensure
 ...
 * All lifecycle transitions (initialize/shutdown) are synchronized and use
 * atomic boolean to prevent concurrent modifications.
 * <p>
 * <strong>Error Handling:</strong>
 * If any component fails to initialize, the entire initialization fails.
 * Shutdown continues even if individual components fail, logging errors
 * but not propagating exceptions.
 *
 * @see Initializable
...
public class LifecycleManager {
     ...
     * <p>
     * Creates manager instances for each component type. Managers are
     * responsible for loading and initializing their respective components.
     *
     ...
     * Uses {@link CompletableFuture#allOf} to wait for all parallel
     * initialization tasks. If any fails, all are cancelled.
     * <p>
     * <strong>State Management:</strong>
     * Sets running flag atomically. On failure, resets flag to allow retry.
     *
    ...
    public synchronized void initialize() {
        ...
        try {
            ...
            throw new CheshireException("Failed to initialize LifecycleManager - one or more components failed to initialize", e);
        }
    }
     ...
     * <p>
     * <strong>Error Handling:</strong>
     * Continues shutdown even if individual components fail. Errors are
     * logged but not propagated to ensure all cleanup attempts are made.
     * <p>
     * <strong>Thread Pool Cleanup:</strong>
     * Executor service is shut down gracefully with timeout. If tasks
     * don't complete in time, forces immediate termination.
     *
    ...
    public synchronized void shutdown() {
        ...
        try {
            ...
            throw new CheshireException("Failed to shutdown LifecycleManager - error during component shutdown or executor termination", e);
        }
    }
     ...
     * <p>
     * Returns {@code true} only after successful initialization and
     ...
     * <p>
     * Components are registered with their initialization phase to ensure
     ...
     * <p>
     * The capability manager holds all business capabilities (domains)
     * with their actions and pipelines.
     *
     ...
     * <p>
     * The source provider manager holds all data sources (databases,
     * APIs, file systems) available to the framework.
     *
     ...
     * <p>
     * The query engine manager holds all query engines (JDBC, Calcite)
     ...
     * <p>
     * Used to maintain registration order and phase information for
