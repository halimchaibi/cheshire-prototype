 ...
 * <p>
 * A query engine processes structured queries against data sources. It abstracts
 * the execution model, allowing different implementations (JDBC, Calcite, custom DSLs)
 ...
 * Uses Java's {@link ServiceLoader} to discover {@link QueryEngineFactory}
 ...
 * Engines implement {@link AutoCloseable}. Manager ensures proper cleanup during
 ...
 * All operations thread-safe via underlying {@link Registry}.
 *
 * @see QueryEngine
...
public final class QueryEngineManager implements Initializable {
    ...
    private final Registry<QueryEngine<?, ?>> registry = new Registry<>(
            "QueryEngine",
            engine -> {
                try {
                    if (engine != null) {
     ...
     *
     * <p><strong>Warning:</strong> Use {@link #shutdown()} for proper cleanup.</p>
     */
    public void clear() {
        registry.clear();
    }
     ...
     * Config specifies the engine factory by class name, providing explicit
     * control over which implementation processes queries.
     *
     * @throws IllegalStateException if no factory found for a configured engine
    ...
    public void initialize() {
        ...
        Map<String, QueryEngineFactory> factories =
                loader.stream()
        ...
        engines.forEach((engineName, engineDef) -> {
            ...
            QueryEngineConfig config =
                    factory.adapter().adapt(engineName, engineDef);
            ...
            try {
                ...
                throw new IllegalStateException(
                        "Failed to initialize QueryEngine: " + engineName, e
                );
            }
        });
    }
     ...
     *
     * <p>This method closes all engines (best-effort) and then removes them
     * from the registry. Exceptions during shutdown are logged but do not
     * prevent other engines from being closed.</p>
     */
    @Override
