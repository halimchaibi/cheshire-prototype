 ...
 * <p>
 * A source provider abstracts access to external data sources (databases, APIs, file systems). It handles connections,
 * authentication, and resource lifecycle. Examples: JDBC connections, HTTP APIs, S3 buckets.
 * <p>
 * <strong>Initialization via SPI:</strong> This manager uses Java's {@link ServiceLoader} SPI mechanism to discover
 * {@link SourceProviderFactory} implementations at runtime. Factories create providers from configuration.
 * <p>
 ...
 * </ol>
 * <p>
 * <strong>Lifecycle Management:</strong> Providers implement {@link AutoCloseable}. The manager ensures proper cleanup
...
public final class SourceProviderManager implements Initializable {
    ...
    private final Registry<SourceProvider<?, ?>> registry = new Registry<>("SourceProvider", provider -> {
        try {
            if (provider != null) {
     ...
     *
     * @param name
     *            the unique name for the provider, must not be null or blank
     * @param provider
     *            the source provider instance, must not be null
     * @throws RegistryException
     *             if a provider with the same name is already registered
     * @throws IllegalArgumentException
     *             if name is null/blank or provider is null
     */
     ...
     *
     * @param name
     *            the source name, must not be null or blank
     * @return the provider instance, never null
     * @throws RegistryException
     *             if no provider is registered with the given name
     * @throws IllegalArgumentException
     ...
     *
     * <p>
     * <strong>Warning:</strong> Use {@link #shutdown()} if proper cleanup is needed.
     * </p>
     */
    public void clear() {
        registry.clear();
     ...
     * </ol>
     * <p>
     * <strong>Factory Resolution:</strong> The config specifies the provider factory by class name. This approach
     * provides explicit control over which implementation handles each source.
     *
     * @throws IllegalStateException
     *             if no factory found for a configured source or if provider creation fails
     */
    @Override
    public void initialize() {
        ...
        ServiceLoader<SourceProviderFactory> loader = ServiceLoader.load(SourceProviderFactory.class,
                SourceProviderFactory.class.getClassLoader());

        // Cache factories by class name (stable, no guessing)
        Map<String, SourceProviderFactory> factories = loader.stream().map(ServiceLoader.Provider::get)
        ...
        sources.forEach((sourceName, sourceDef) -> {
            ...
            String factoryClass = config.getSources().get(sourceName).getProvider();

            SourceProviderFactory factory = factories.get(factoryClass);

            if (factory == null) {
                throw new IllegalStateException("No SourceProviderFactory found for: " + factoryClass);
            }

            SourceConfig config = factory.adapter().adapt(sourceName, sourceDef);

            try {
                SourceProvider<?, ?> source = factory.create(config);
                register(sourceName, source);
            } catch (Exception e) {
                throw new IllegalStateException("Failed to initialize SourceProvider: " + sourceName, e);
            }
        });
    }
