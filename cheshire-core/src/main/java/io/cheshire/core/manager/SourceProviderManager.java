...
public final class SourceProviderManager implements Initializable {
  ...
  public void initialize() {
    ...
    sourceConfigs.entrySet().stream()
        .filter(entry -> hasFactory(entry.getValue()))
        .forEach(entry -> initializeSourceProvider(factories, entry.getKey(), entry.getValue()));
  }

  private boolean hasFactory(CheshireConfig.Source sourceDef) {
    return sourceDef.getFactory() != null && !sourceDef.getFactory().isBlank();
  }

  private void initializeSourceProvider(
      Map<String, SourceProviderFactory> factories,
      String sourceName,
      CheshireConfig.Source sourceDef) {
