 ...
 *
 * <p>Immutable input container for pipeline operations, carrying both the primary data and
 * associated metadata through pipeline stages.
 *
 * <h3>Core Responsibilities</h3>
 *
 * <ul>
 *   <li><b>Data Transport:</b> Carries the primary payload through pipeline transformations.
 *   <li><b>Metadata Propagation:</b> Maintains contextual information (e.g., content type,
 *       encoding, timestamps).
 *   <li><b>Immutability:</b> Ensures thread-safe passage through concurrent pipeline stages.
 ...
 * MaterializedInput input = MaterializedInput.of(Map.of("text", "Hello World", "tokens", List.of("Hello", "World")),
 *         Map.of("source", "user_input", "timestamp", Instant.now()));
 *
 * // Safe data access
 * input.getData("text", String.class).ifPresent(text -> logger.info("Processing: {}", text));
 *
 * // Functional transformation
 * MaterializedInput enriched = input.withData("processed", true).withMetadata("stage", "tokenization");
 *
 ...
 * @since 1.0.0
 */
public record MaterializedInput(Map<String, Object> data, Map<String, Object> metadata)
   ...
   */
  public MaterializedInput {
    data = (data != null && !data.isEmpty()) ? new LinkedHashMap<>(data) : new LinkedHashMap<>();

    metadata =
        (metadata != null && !data.isEmpty())
   ...
   */
  public static MaterializedInput of(
   ...
   *
   * <p>This method is required by {@link CanonicalInput} and enables pipeline stages to create
   * modified versions without mutating the original.
   *
