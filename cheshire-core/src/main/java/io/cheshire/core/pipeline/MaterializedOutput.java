 ...
 *
 * <p>Immutable output container for pipeline operations, carrying both the resulting data and
 * associated metadata from pipeline stages.
 *
 * <h3>Core Responsibilities</h3>
 *
 * <ul>
 *   <li><b>Result Transport:</b> Carries the processed output from pipeline transformations.
 *   <li><b>Metadata Propagation:</b> Maintains processing information (e.g., execution time, stage
 *       metrics, errors).
 *   <li><b>Immutability:</b> Ensures thread-safe consumption of pipeline results.
 *   <li><b>Functional Transformation:</b> Supports copy-on-write pattern via {@link #copy(Map,
 *       Map)}.
 * </ul>
 *
 * <h3>Mutability Model</h3>
 *
 * <p>This record follows a <b>fully immutable</b> design:
 *
 * <ul>
 *   <li><b>Defensive Copying:</b> Maps are copied via {@link Map#copyOf(Map)} in the constructor.
 *   <li><b>Unmodifiable Views:</b> All accessors return unmodifiable maps.
 *   <li><b>Functional Updates:</b> Use {@link #withData(String, Object)} or {@link
 *       #withMetadata(String, Object)} to create new instances with modifications.
 * </ul>
 *
 * <h3>Usage Example</h3>
 *
 ...
 * </table>
 *
 * @param data Immutable map containing the pipeline result data.
 * @param metadata Immutable map containing processing metadata and metrics.
 * @author Cheshire Framework
 * @since 1.0.0
 */
public record MaterializedOutput(Map<String, Object> data, Map<String, Object> metadata)
    implements CanonicalOutput<MaterializedOutput> {
  ...
  /**
   * Compact constructor of unmodifiable views of LinkedHashMaps to preserve insertion order
   * ensuring immutability and null safety.
   *
   * <p>Both {@code data} and {@code metadata} are defensively copied using {@link Map#copyOf(Map)},
   * which creates immutable maps. Null inputs are converted to empty immutable maps.
   *
   ...
   */
  public static MaterializedOutput of(
      LinkedHashMap<String, Object> data, LinkedHashMap<String, Object> metadata) {
  ...
  public static MaterializedOutput error(String errorMessage) {
    return new MaterializedOutput(
        new LinkedHashMap<>(), new LinkedHashMap<>(Map.of("error", errorMessage)));
  }
   ...
   *
   * <p>This method is required by {@link CanonicalOutput} and enables pipeline stages to create
   * modified results without mutating the original.
   *
   ...
   *
   * <p>Conventionally, errors are indicated by an "error" key in metadata.
   *
   ...
   *
   * <p>This method implements functional mutation by creating a new instance with all existing data
   * plus the new entry. If the key exists, it will be replaced in the new instance.
   *
   * @param key The data key
   ...
   *
   * <p>If the key exists, it will be replaced in the new instance.
   *
   * @param key The metadata key
