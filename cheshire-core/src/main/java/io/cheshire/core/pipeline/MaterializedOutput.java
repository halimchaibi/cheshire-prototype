 ...
 * <p>Immutable output container for pipeline operations, carrying both the resulting data
 * and associated metadata from pipeline stages.</p>
 *
 ...
 * MaterializedOutput output = MaterializedOutput.of(
 *     Map.of("result", "processed text", "confidence", 0.95),
 *     Map.of("stage", "completion", "duration_ms", 150L)
 * );
 *
 * // Safe data access
 * output.getData("result", String.class)
 *     .ifPresent(result -> logger.info("Result: {}", result));
 *
 * // Functional enrichment
 * MaterializedOutput enriched = output
 *     .withData("validated", true)
 *     .withMetadata("validator", "schema_v2");
 *
 ...
 *     output.getMetadata("error", String.class)
 *         .ifPresent(error -> logger.error("Pipeline error: {}", error));
 * }
 * }</pre>
 *
 * <h3>MaterializedOutput vs MaterializedInput</h3>
 * <table border="1">
 * <tr><th>Aspect</th><th>MaterializedOutput</th><th>MaterializedInput</th></tr>
 * <tr><td>Purpose</td><td>Pipeline results</td><td>Pipeline inputs</td></tr>
 * <tr><td>Data Content</td><td>Processed/transformed data</td><td>Raw/source data</td></tr>
 * <tr><td>Metadata Content</td><td>Processing metrics, errors</td><td>Context, source info</td></tr>
 * <tr><td>Usage</td><td>Returned from stages</td><td>Passed to stages</td></tr>
 * </table>
 *
...
public record MaterializedOutput(
    ...
    /**
     * Compact constructor of unmodifiable views of LinkedHashMaps to preserve insertion order ensuring immutability and null safety.
     * <p>
    ...
    public static MaterializedOutput error(String errorMessage) {
        return new MaterializedOutput(
                new LinkedHashMap<>(),
                new LinkedHashMap<>(Map.of("error", errorMessage))
        );
    }
     ...
     * <p>
     * This method is required by {@link CanonicalOutput} and enables
     * pipeline stages to create modified results without mutating the original.
     * </p>
     *
    ...
    public MaterializedOutput withError(String errorMessage, Throwable exception) {
        return withMetadata(Map.of(
                "error", errorMessage,
                "exception_type", exception.getClass().getName(),
