 ...
 * <p>
 * This factory builds {@link PipelineProcessor} instances from {@link PipelineConfig}
 ...
 * <p>
 * The factory uses reflection to instantiate pipeline step implementations from
 * fully qualified class names specified in configuration. It attempts two
 * instantiation strategies:
 * <ol>
 *   <li><strong>Config Constructor:</strong> {@code Constructor(Map<String, Object>)}
 *       - Allows steps to receive configuration (template, name, etc.)</li>
 *   <li><strong>Default Constructor:</strong> {@code Constructor()}
 *       - Fallback for simple stateless steps</li>
 * </ol>
 * <p>
 ...
 * PipelineProcessor<MaterializedInput, MaterializedOutput> pipeline =
 *     PipelineFactory.build("createAuthor", config);
 *
 * // Execute pipeline
...
public interface PipelineFactory {
     ...
     * The factory ensures type compatibility by casting steps to their
     * appropriate interfaces ({@link PreProcessor}, {@link Executor}, {@link PostProcessor}).
     *
     ...
     * </ol>
     * <p>
     * <strong>Configuration Map:</strong>
     * Contains:
     * <ul>
    ...
    private static <T> T instantiate(String pipelineName, PipelineConfig.Step step, Class<T> targetType) {
        ...
        final Map<String, Object> config = Map.of(
                "template", Optional.ofNullable(step.getTemplate()).orElse(""),
                "name", stepName
        );
        ...
        try {
            ...
            if (!targetType.isAssignableFrom(implementationClass)) {
                throw new IllegalArgumentException("Class %s does not implement %s"
                        .formatted(impl, targetType.getSimpleName()));
            }
            ...
            throw new RuntimeException("Pipeline '%s' step '%s': Failed to initialize %s"
                    .formatted(pipelineName, stepName, impl), e);
        }
    }
    ...
    private static <T extends Step<?, ?>> T instantiate2(PipelineConfig.Step configStep, Class<T> targetType) {
        ...
        try {
            ...
            throw new RuntimeException("Step '%s': Failed to assemble step '%s' - %s".formatted(stepName, impl, e.getMessage()), e);
        }
    }
}
