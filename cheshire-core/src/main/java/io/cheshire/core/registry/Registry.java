 ...
 *
 * <p>This class provides a consistent, thread-safe mechanism for registering, retrieving, and
 * managing named instances. It serves as a base implementation that can be used directly or
 * extended for specific use cases.
 *
 * <p><strong>Key Features:</strong>
 *
 * <ul>
 *   <li>Thread-safe operations using {@link ConcurrentHashMap}
 *   <li>Duplicate registration prevention
 *   <li>Optional resource cleanup on shutdown
 *   <li>Comprehensive logging
 *   <li>Null-safety with validation
 *   <li>Immutable snapshots of registry state
 * </ul>
 *
 * <p><strong>Thread Safety:</strong> All operations are thread-safe and can be safely called from
 ...
 * }</pre>
 *
 * @param <T> the type of instances managed by this registry
...
public class Registry<T> {
   ...
   *
   * @param registryType a descriptive name for this registry type (used in logging)
   * @throws IllegalArgumentException if registryType is null or blank
   */
   ...
   *
   * <p>The shutdown handler is invoked for each registered instance when {@link #shutdown()} is
   * called. This is useful for registries managing resources that need cleanup (e.g., closing
   * connections).
   *
   * @param registryType a descriptive name for this registry type (used in logging)
   * @param shutdownHandler optional handler for cleaning up instances on shutdown
   * @throws IllegalArgumentException if registryType is null or blank
   */
   ...
   *
   * <p>The name must be unique within this registry. If an instance with the same name is already
   * registered, a {@link RegistryException} is thrown.
   *
   * @param name the unique name for the instance, must not be null or blank
   * @param instance the instance to register, must not be null
   * @throws RegistryException if an instance with the same name is already registered or if the
   *     registry has been shutdown
   * @throws IllegalArgumentException if name is null/blank or instance is null
   */
  public void register(String name, T instance) {
    ...
    if (existing != null) {
      String errorMsg =
          String.format(
              "%s already registered with name '%s'. Existing instance: %s",
              registryType, name, existing.getClass().getName());
      log.error(errorMsg);
      throw new RegistryException(errorMsg);
    }

    log.info(
        "Registered {} '{}' (type: {})", registryType, name, instance.getClass().getSimpleName());
  }

  /**
   * Retrieves the instance registered with the given name.
   *
   * @param name the instance name, must not be null or blank
   * @return the registered instance, never null
   * @throws RegistryException if no instance is registered with the given name or if the registry
   *     has been shutdown
  ...
  public T get(String name) {
    ...
    if (instance == null) {
      String errorMsg =
          String.format(
              "No %s registered with name '%s'. Available: %s",
              registryType, name, instances.keySet());
      log.error(errorMsg);
      throw new RegistryException(errorMsg);
    }
   ...
   *
   * <p>This is a null-safe alternative to {@link #get(String)} that doesn't throw an exception when
   * the instance is not found.
   *
   * @param name the instance name, must not be null or blank
   * @return an Optional containing the instance if found, empty otherwise
   * @throws RegistryException if the registry has been shutdown
   * @throws IllegalArgumentException if name is null or blank
   */
   ...
   *
   * <p>The returned map is a snapshot of the registry at the time of the call. Changes to the
   * registry after this call will not be reflected in the returned map.
   *
   ...
   *
   * <p><strong>Note:</strong> This method does not invoke the shutdown handler on the removed
   * instance. If cleanup is needed, the caller must handle it.
   *
   * @param name the instance name, must not be null or blank
   ...
   *
   * <p>This is useful when you need both to remove an instance and obtain it for cleanup purposes.
   *
   * @param name the instance name, must not be null or blank
   ...
   *
   * <p><strong>Warning:</strong> This operation removes all registered instances from the registry
   * but does NOT invoke the shutdown handler. Use {@link #shutdown()} if proper cleanup is needed.
   *
   * <p>This is primarily useful for testing or application reload scenarios. Use with caution in
   * production code.
   *
   * @throws RegistryException if the registry has been shutdown
   */
   ...
   *
   * <p>If a shutdown handler was provided during construction, it will be invoked for each
   * registered instance. The shutdown process is best-effort: if an exception occurs while
   * processing an instance, it is logged but does not prevent processing of other instances.
   *
   * <p>After shutdown, the registry cannot be used for any operations except {@link #isShutdown()}.
   *
  ...
  public String toString() {
    return String.format(
        "%s{type='%s', instances=%s, shutdown=%s}",
        getClass().getSimpleName(), registryType, instances.keySet(), shutdown);
  }
