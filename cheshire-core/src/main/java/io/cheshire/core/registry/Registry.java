 ...
 *
 * <p>This class provides a consistent, thread-safe mechanism for registering, retrieving,
 * and managing named instances. It serves as a base implementation that can be used
 * directly or extended for specific use cases.</p>
 *
 ...
 * </ul>
 *
 * <p><strong>Thread Safety:</strong> All operations are thread-safe and can be safely
...
public class Registry<T> {
     ...
     *
     * <p>The shutdown handler is invoked for each registered instance when
     * {@link #shutdown()} is called. This is useful for registries managing
     * resources that need cleanup (e.g., closing connections).</p>
     *
     * @param registryType    a descriptive name for this registry type (used in logging)
     ...
     *
     * <p>The name must be unique within this registry. If an instance with the same
     * name is already registered, a {@link RegistryException} is thrown.</p>
     *
     ...
     * @throws RegistryException        if an instance with the same name is already registered
     *                                  or if the registry has been shutdown
     ...
     * @throws RegistryException        if no instance is registered with the given name
     *                                  or if the registry has been shutdown
     ...
     *
     * <p>This is a null-safe alternative to {@link #get(String)} that doesn't throw
     * an exception when the instance is not found.</p>
     *
     * @param name the instance name, must not be null or blank
     ...
     *
     * <p>The returned map is a snapshot of the registry at the time of the call.
     * Changes to the registry after this call will not be reflected in the returned map.</p>
     *
     ...
     *
     * <p><strong>Note:</strong> This method does not invoke the shutdown handler
     * on the removed instance. If cleanup is needed, the caller must handle it.</p>
     *
     * @param name the instance name, must not be null or blank
     ...
     *
     * <p>This is useful when you need both to remove an instance and obtain it
     ...
     *
     * <p><strong>Warning:</strong> This operation removes all registered instances
     * from the registry but does NOT invoke the shutdown handler. Use {@link #shutdown()}
     * if proper cleanup is needed.</p>
     *
     * <p>This is primarily useful for testing or application reload scenarios.
     * Use with caution in production code.</p>
     *
     * @throws RegistryException if the registry has been shutdown
     */
    public void clear() {
     ...
     *
     * <p>If a shutdown handler was provided during construction, it will be invoked
     * for each registered instance. The shutdown process is best-effort: if an
     * exception occurs while processing an instance, it is logged but does not
     * prevent processing of other instances.</p>
     *
     * <p>After shutdown, the registry cannot be used for any operations except
     * {@link #isShutdown()}.</p>
     *
    ...
    public String toString() {
        return String.format("%s{type='%s', instances=%s, shutdown=%s}",
