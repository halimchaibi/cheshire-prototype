 ...
 * <p>
 * Implementations of this interface are discovered via Java's Service Provider Interface (SPI). The factory class name
 * is specified in the transport configuration, and the framework dynamically loads and instantiates it.
 * <p>
 ...
 * <p>
 * The {@link #load(String)} static method provides reflection-based loading with clear error messages for common
 * failures (class not found, wrong type, instantiation errors).
 *
 * @see CheshireServer
...
public interface CheshireServerFactory {
     ...
     * </ol>
     * <p>
     * <strong>Error Handling:</strong> Provides clear error messages for common failures:
     * <ul>
     ...
     *
     * @param className
     *            fully qualified class name of the factory implementation
    ...
    static CheshireServerFactory load(String className) {
        try {
            ...
            if (!CheshireServerFactory.class.isAssignableFrom(clazz)) {
                throw new IllegalArgumentException("Class " + className + " does not implement CheshireServerFactory");
            }
            return (CheshireServerFactory) clazz.getDeclaredConstructor().newInstance();
        } catch (ClassNotFoundException e) {
            log.error("Factory class not found in classpath: '{}'. Check your YAML config or dependencies.", className,
                    e);
     ...
     * </ul>
     * <p>
     * <strong>Server State:</strong> The returned server should be in the NEW state, ready for
     * {@link CheshireServer#init()} and {@link CheshireServer#start()} calls.
     *
     * @param capability
     *            capability configuration with transport settings
     * @param binding
     *            exposure binding string (e.g., "/api/v1", "mcp-stdio")
     * @param dispatcher
     *            dispatcher for routing and processing requests
