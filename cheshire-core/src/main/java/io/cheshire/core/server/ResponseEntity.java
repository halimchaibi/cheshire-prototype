 ...
 * *
 *
 * <p>A <b>Sealed Algebraic Data Type (ADT)</b> representing the immutable outcome of a Cheshire
 * operation. This interface is the standard return type for the {@code CheshireDispatcher} and all
 * {@code Action} implementations. *
 *
 * <h3>Architectural Intent</h3>
 *
 * <p>By leveraging Java's {@code sealed} interface and {@code record} features, this type ensures
 * <b>exhaustive error handling</b>. Callers are forced by the compiler to handle both the {@link
 * Success} and {@link Failure} cases, eliminating the risk of unhandled runtime exceptions or
 * "null-check" programming styles. *
 *
...
public sealed interface ResponseEntity permits ResponseEntity.Success, ResponseEntity.Failure {
  ...
  /**
   * Factory method to create a {@link Success} response with empty metadata. * @param data The
   * operational payload.
   *
   ...
   *
   * @param data The operational payload.
   * @param metadata Map of non-domain context (e.g., pagination, trace IDs).
   * @return A Success instance.
   */
  ...
  /**
   * Factory method to create a {@link Failure} response including the original Throwable. * @param
   * status The logical error category.
   *
   * @param error The cause of the failure (can be null).
  ...
  /**
   * Factory method to create a {@link Failure} response with a custom message. * @param status The
   * logical error category.
   *
  ...
  /**
   * Defines the logical categories for Cheshire operations. These are mapped to protocol-specific
   * codes (HTTP or MCP JSON-RPC) by the adapter layer.
   */
  enum Status {
  ...
  /**
   * Represents a successful operation outcome. * @param data The primary payload produced by the
   * Action.
   *
   * @param metadata Contextual key-value pairs (e.g., execution metrics).
   */
  record Success(Object data, Map<String, Object> metadata) implements ResponseEntity {}

  /**
   * Represents a failed operation outcome. * @param status The category of failure for routing and
   * logic.
   *
   * @param error The captured exception, if any.
   * @param message A descriptive summary of why the operation failed.
   */
