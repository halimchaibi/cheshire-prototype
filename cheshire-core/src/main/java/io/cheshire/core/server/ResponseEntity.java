 ...
 * * <p>A <b>Sealed Algebraic Data Type (ADT)</b> representing the immutable outcome of a Cheshire operation.
 * This interface is the standard return type for the {@code CheshireDispatcher} and all {@code Action}
 * implementations.</p>
 * * <h3>Architectural Intent</h3>
 * <p>By leveraging Java's {@code sealed} interface and {@code record} features, this type ensures
 * <b>exhaustive error handling</b>. Callers are forced by the compiler to handle both the
 * {@link Success} and {@link Failure} cases, eliminating the risk of unhandled runtime exceptions
 * or "null-check" programming styles.</p>
...
public sealed interface ResponseEntity permits ResponseEntity.Success, ResponseEntity.Failure {
    ...
    /**
     * Factory method to create a {@link Success} response with empty metadata.
    ...
    /**
     * Factory method to create a {@link Failure} response including the original Throwable.
     * * @param status The logical error category.
     *
    ...
    /**
     * Factory method to create a {@link Failure} response with a custom message.
     * * @param status  The logical error category.
     *
    ...
    /**
     * Defines the logical categories for Cheshire operations.
     * These are mapped to protocol-specific codes (HTTP or MCP JSON-RPC) by the adapter layer.
     */
    enum Status {
        /**
     ...
     * * @param data     The primary payload produced by the Action.
     *
     ...
     * * @param status  The category of failure for routing and logic.
     *
