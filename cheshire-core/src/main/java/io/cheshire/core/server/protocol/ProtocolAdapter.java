 ...
 * This interface is responsible for the bidirectional mapping of data, ensuring that the {@code
 * CheshireDispatcher} remains agnostic of the transport layer. *
 *
 * <h3>The Bidirectional Contract</h3>
 *
 * <ul>
 *   <li><b>Ingress (toRequestEnvelope):</b> Transforms a protocol-specific request (e.g., {@code
 *       HttpServletRequest} or an MCP {@code CallToolRequest}) into the standardized {@link
 *       RequestEnvelope}.
 *   <li><b>Egress (fromProcessingResult):</b> Transforms the internal {@link ResponseEntity} back
 *       into a format the client understands (e.g., a JSON-RPC response or a REST JSON body).
 * </ul>
 *
 * <h3>Error Handling</h3>
 *
 * <p>Implementations must catch protocol-specific parsing or validation errors and rethrow them as
 * {@link ProtocolAdapterException}. This allows the jetty infrastructure to handle mapping failures
 * consistently across different transports.
 *
 ...
 * @param <O> The Outbound response type expected by the transport layer. * @author Cheshire
 *     Framework
 * @since 1.0.0
 */
public interface ProtocolAdapter<I, O> {
   ...
   * @throws ProtocolAdapterException if the transformation fails due to malformed input or
   *     incompatible schema.
   */
   ...
   *
   * <p>This method uses type inference to determine the appropriate return type (O)
   *
  ...
  /**
   * Identifies the protocol type this adapter is designed to handle. * @return A string identifier
   * (e.g., "MCP", "REST", "GRPC").
   */
  String getProtocolType();

  /**
   * Defines the MIME types or content types supported by this adapter. Defaults to {@code
   * application/json}.
   *
