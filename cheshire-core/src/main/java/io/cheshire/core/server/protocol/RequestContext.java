 ...
 * cross-cutting concerns such as identity, security, observability, and transient
 * attributes through the Cheshire pipeline.</p>
 *
 * <h3>Core Responsibilities</h3>
 * <ul>
 * <li><b>Identity:</b> Tracks the {@code sessionId} and {@code userId} across asynchronous boundaries.</li>
 * <li><b>Observability:</b> Stores {@code transportHeaders} for distributed tracing (e.g., W3C Trace Context, Zipkin).</li>
 * <li><b>Governance:</b> Maintains a {@code securityContext} for Role-Based Access Control (RBAC).</li>
 * <li><b>Resilience:</b> Holds an optional {@code deadline} (TTL) to enforce timeouts for long-running AI operations.</li>
 * </ul>
 *
 * <h3>Lifecycle & Mutability</h3>
 * <p>This record employs a hybrid mutability model to balance safety with flexibility:</p>
 * <ul>
 * <li><b>Immutable Context:</b> {@code securityContext} and {@code transportHeaders} are exposed as
 * unmodifiable maps to prevent accidental modification of security-sensitive data.</li>
 * <li><b>Mutable Attributes:</b> The {@code attributes} map uses {@link ConcurrentHashMap} internally
 * to allow thread-safe enrichment as the request passes through Interceptors and Filters.</li>
 * <li><b>Defensive Copying:</b> The {@link #attributes()} accessor returns a defensive copy to prevent
 * external code from holding references to the internal mutable state.</li>
 * </ul>
 *
 * <h3>Usage Example</h3>
 * <pre>{@code
 ...
 * context.getAttribute("tenant_id").ifPresent(id ->
 *     logger.info("Processing request for tenant: {}", id)
 * );
 *
 ...
 * synchronized—callers are responsible for thread-safety of the values themselves.</p>
 *
...
public record RequestContext(
     ...
     */
    public RequestContext(String sessionId, String userId, String traceId,
                          Map<String, Object> securityContext,
                          Map<String, String> transportHeaders,
                          ConcurrentMap<String, Object> attributes,
                          Instant arrivalTime,
        ...
        this.transportHeaders = transportHeaders != null
     ...
     * affect the internal state of this RequestContext.
     * </p>
     * <p>
