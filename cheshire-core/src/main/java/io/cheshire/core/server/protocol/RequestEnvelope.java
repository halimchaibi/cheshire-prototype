 ...
 * "Shipping Container" for every inbound request, encapsulating identity, domain routing,
 * payload data, and stateful context.</p>
 ...
 * <li>{@code capability}: The domain-level namespace (e.g., "tools", "prompts") used to locate the correct Registry.</li>
 * <li>{@code action}: The specific operation to execute within the designated capability.</li>
 * </ul>
 * * <h3>Design Invariants</h3>
 * <p>The <b>Compact Constructor</b> enforces strict validation rules:</p>
 * <ul>
 ...
 * metadata or context is not immediately available during protocol ingestion.</p>
 * * <pre>{@code
 * RequestEnvelope envelope = RequestEnvelope.of(
 * "req-123",
 * "tools",
 * "query-db",
 * myPayload
 ...
 * @param receivedAt       The exact timestamp when the request entered the Cheshire system.
...
public record RequestEnvelope(
     ...
     */
    public static RequestEnvelope of(String id, String cap, String act, RequestPayload payload, ProtocolMetadata protocolMetadata) {
        return new RequestEnvelope(id, cap, act, protocolMetadata, payload, RequestContext.empty(), java.time.Instant.now());
    }
}
