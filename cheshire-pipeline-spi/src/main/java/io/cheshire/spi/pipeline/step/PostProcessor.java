 ...
 * <p>
 * PostProcessors refine and format output after the executor completes.
 * They transform {@link CanonicalOutput} to {@link CanonicalOutput}, preparing
 ...
 * <p>
 * Multiple postprocessors execute sequentially, each receiving the output
 * of the previous one:
 * <pre>
 ...
 * <p>
 * Input and output types are the same ({@code O → O}), ensuring type safety
 * through the postprocessing chain.
 * <p>
 * <strong>Performance Consideration:</strong>
 * <p>
 * Postprocessors run after the executor completes. For expensive operations
 * (sorting large datasets, complex calculations), consider moving logic to
 * the executor or database layer.
 *
