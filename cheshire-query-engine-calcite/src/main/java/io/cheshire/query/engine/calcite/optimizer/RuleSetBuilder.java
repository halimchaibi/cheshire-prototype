...
public class RuleSetBuilder {
  ...
  private void analyzeSourceCapabilities() {
    ...
    boolean supportsArrowAdapterRules = false;
    ...
    for (String sourceName : sourceNames) {
      ...
      // ElasticSearch supports filter and projection
        ...
        supportsFilterPushdown = true;
        supportsProjectionPushdown = true;
      } else if ("ARROW".equalsIgnoreCase(sourceType)) {
        supportsFilterPushdown = true;
        supportsProjectionPushdown = true;
        supportsArrowAdapterRules = true;
      }
    ...
    if (supportsArrowAdapterRules) {
      addArrowAdapterRules();
    }
  }
    ...
    try {
      ...
      if (schemaManager.rootSchema() != null) {
        ...
        if (schema != null) {
          ...
          } else if (schemaClassName.contains("ARROW")) {
            return "ARROW";
  ...
  private void addArrowAdapterRules() {
    log.debug("Adding Arrow adapter rules");
    ruleSetBuilder
        .addRule(ArrowRules.FILTER_SCAN)
        .addRule(ArrowRules.PROJECT_SCAN)
        .addRule(ArrowRules.TO_ENUMERABLE);
  }
}
