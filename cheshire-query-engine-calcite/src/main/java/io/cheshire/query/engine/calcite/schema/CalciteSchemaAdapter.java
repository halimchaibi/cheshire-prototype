 ...
 * <p>
 * Currently supports JDBC source providers by creating Calcite JdbcSchema
 * instances from the JDBC connection configuration.
 */
public class CalciteSchemaAdapter {
     ...
     *
     * @param schemaName     the name for the schema in Calcite
     * @param sourceProvider the source provider to adapt
     * @param rootSchema     the root schema to add the schema to
     * @return the Calcite Schema instance
     * @throws SourceProviderException if adaptation fails
     */
    public Schema createSchema(String schemaName, SourceProvider<?, ?> sourceProvider, SchemaPlus rootSchema) throws SourceProviderException {
        ...
        throw new SourceProviderException("Unsupported source provider type for Calcite: " + sourceProvider.config().type());
    }

    /**
     * Creates a Calcite JdbcSchema from a JDBC source provider.
     */
    private Schema createJdbcSchema(String schemaName, SourceProvider<?, ?> sourceProvider, SchemaPlus rootSchema) throws SourceProviderException {
        ...
        try {
            ...
            if (!(sourceProvider.config() instanceof JdbcDataSourceConfig jdbcConfig)) {
                throw new SourceProviderException("Source provider config is not a JdbcDataSourceConfig: " + sourceProvider.config().getClass());
            }
