...
public class ResultTransformer {
  ...
  public QueryEngineResult transform(ResultSet resultSet) throws SQLException {
    ...
    while (resultSet.next()) {
      Map<String, Object> row = new LinkedHashMap<>();
      for (int i = 1; i <= columnCount; i++) {
        row.put(columnNames.get(i - 1), valueAt(resultSet, i));
      }
      rows.add(row);
    }
  ...
  public QueryEngineResult transform(ResultSet resultSet, RelNode plan) throws SQLException {
    ...
    while (resultSet.next()) {
      ...
      for (int i = 0; i < columnNames.size(); i++) {
        row.put(columnNames.get(i), valueAt(resultSet, i + 1)); // JDBC is 1-indexed
      }
      rows.add(row);
    }
  ...
  private Object valueAt(final ResultSet resultSet, final int columnIndex) throws SQLException {
    try {
