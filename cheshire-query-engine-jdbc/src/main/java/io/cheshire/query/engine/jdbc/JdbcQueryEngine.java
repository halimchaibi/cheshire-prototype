 ...
 *
 * <p>This engine executes SQL queries directly against a JDBC data source without
 * query planning or optimization. It's suitable for:
 * <ul>
 ...
 * </p>
 *
 * <p><strong>Thread Safety:</strong> This implementation is not thread-safe for manager
 * operations (open/close). Query execution may be thread-safe depending on the underlying
 * JdbcDataSourceProvider implementation. External synchronization is required for concurrent access.</p>
 *
 * <p><strong>Resource Management:</strong> This class implements {@link AutoCloseable}.
 * Always call {@link #close()} or use try-with-resources to ensure proper cleanup.</p>
 *
...
public class JdbcQueryEngine implements QueryEngine<SqlQueryRequest, JdbcDataSourceProvider> {
     ...
     *
     * @param config the engine configuration, must not be null and must be valid
    ...
    public String explain(SqlQueryRequest query) throws QueryExecutionException {
        if (!opened) {
            throw new QueryExecutionException(String.format("Query engine '%s' is not open. Call open() before explaining queries.", name));
        }
     ...
     * {@link #execute(SqlQueryRequest, JdbcDataSourceProvider)} .</p>
     *
     ...
     * @param source the execution context (e.g., schema manager, source provider)
    ...
    public MapQueryResult execute(SqlQueryRequest query, JdbcDataSourceProvider source) throws QueryExecutionException {
        if (!opened) {
            try {
                ...
                throw new QueryExecutionException("Failed to open query engine before executing the query %s".formatted(query.sql()), e);
            } catch (Exception e) {
                throw new QueryExecutionException("Unexpected error opening query engine before executing the query %s".formatted(query.sql()), e);
            }
        }
        ...
        try {
            ...
            log.info("Query executed successfully on engine '{}' in {}ms, returned {} rows", name, duration, queryResult.rowCount());

            return queryResult;

        } catch (SourceProviderException e) {
            String errorMsg = String.format("Failed to execute query on engine '%s' against source '%s' for query '%s': %s", name, source.config().name(), sql, e.getMessage());
            log.error(errorMsg, e);
            throw new QueryExecutionException(errorMsg, e);
        } catch (Exception e) {
            String errorMsg = String.format("Unexpected error executing query on engine '%s' for query '%s': %s", name, sql, e.getMessage());
            log.error(errorMsg, e);
            throw new QueryExecutionException(errorMsg, e);
        }
    }
