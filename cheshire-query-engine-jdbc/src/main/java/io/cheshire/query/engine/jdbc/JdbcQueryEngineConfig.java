 ...
 * </p>
 *
 * <p>The JDBC query engine executes queries directly against a single JDBC data source
 * without query planning or optimization. It's suitable for simple, direct database queries.</p>
 *
 * @param name    the unique name/identifier for this query engine instance
 * @param sources the list of source names (typically contains one source for JDBC engine)
...
public record JdbcQueryEngineConfig(String name, List<String> sources) implements QueryEngineConfig {
     ...
     * @param queryConf the query definition from CheshireConfig
