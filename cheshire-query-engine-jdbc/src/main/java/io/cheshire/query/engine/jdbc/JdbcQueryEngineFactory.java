 ...
 * <p>
 * This factory is discovered via Java's Service Provider Interface (SPI).
 * It's registered in `META-INF/services/io.cheshire.spi.query.engine.QueryEngineFactory`.
 * <p>
 * <strong>Configuration Adaptation:</strong>
 * <p>
 * Converts raw YAML configuration ({@link CheshireConfig.QueryEngine}) into
 * typed {@link JdbcQueryEngineConfig} via the adapter pattern.
 * <p>
 ...
 * @since 1.0.0
 */
public class JdbcQueryEngineFactory implements QueryEngineFactory<JdbcQueryEngineConfig, JdbcQueryEngine, CheshireConfig.QueryEngine> {
     ...
     *
     * @param config typed JDBC engine configuration
     * @return initialized query engine instance
     * @throws QueryEngineException if configuration is invalid or creation fails
     */
    @Override
    ...
    public ConfigAdapter<CheshireConfig.QueryEngine> adapter() {
        return (name, queryConf)
                -> JdbcQueryEngineConfig.from(name, queryConf);
    }
