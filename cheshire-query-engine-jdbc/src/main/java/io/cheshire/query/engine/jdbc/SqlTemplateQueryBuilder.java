 ...
 * <p>
 * Converts declarative JSON templates (DSL_QUERY) into executable SQL queries with parameterized values. Supports
 ...
 * <p>
 * Uses named parameters (`:paramName`) for SQL injection prevention. Parameters are extracted from request and
...
public final class SqlTemplateQueryBuilder {
     ...
     */
    public static SqlQueryRequest buildQuery(String templateJson, Map<String, Object> requestParams)
        ...
        String operation = Optional.ofNullable(template.get("operation")).map(n -> n.asText().toUpperCase())
                .orElse("SELECT");
     ...
     */
    private static SqlQueryRequest buildSelect(JsonNode template, Map<String, Object> requestParams)
            throws QueryEngineException {
     ...
     * Builds Common Table Expressions (WITH clause).
     */
    private static void buildCtes(StringBuilder sql, JsonNode template, Map<String, Object> requestParams,
            Map<String, Object> params) throws QueryEngineException {
     ...
     */
    private static String buildSubquery(JsonNode subqueryTemplate, Map<String, Object> requestParams,
            Map<String, Object> params) throws QueryEngineException {
     ...
     */
    private static SqlQueryRequest buildInsert(JsonNode template, Map<String, Object> requestParams)
            throws QueryEngineException {
        StringBuilder sql = new StringBuilder();
        Map<String, Object> params = new LinkedHashMap<>();
        ...
        for (JsonNode column : columns) {
            ...
            handleValueMapping(column, requestParams, params, valuePlaceholders,
     ...
     */
    private static SqlQueryRequest buildUpdate(JsonNode template, Map<String, Object> requestParams)
            throws QueryEngineException {
        StringBuilder sql = new StringBuilder();
        Map<String, Object> params = new LinkedHashMap<>();
     ...
     */
    private static SqlQueryRequest buildDelete(JsonNode template, Map<String, Object> requestParams)
            throws QueryEngineException {
        StringBuilder sql = new StringBuilder();
        Map<String, Object> params = new LinkedHashMap<>();
    ...
    private static void handleValueMapping(JsonNode node, Map<String, Object> requestParams, Map<String, Object> params,
     ...
     */
    private static void handleValueMapping(JsonNode node, Map<String, Object> requestParams, Map<String, Object> params,
            List<String> placeholders, boolean isOptional) {
        if (node.has("param")) {
        ...
        if (hasProjection) {
            for (JsonNode proj : template.get("projection")) {
                ...
                if (!requestedFields.isEmpty()) {
                    ...
                    if (!requestedFields.contains(match))
                        continue;
                }
     ...
     */
    private static void buildFilters(StringBuilder sql, JsonNode template, Map<String, Object> requestParams,
     ...
     */
    private static String buildConditions(JsonNode filterNode, Map<String, Object> requestParams,
            Map<String, Object> params) {
        if (filterNode.has("conditions")) {
            String op = filterNode.path("op").asText("AND");
            List<String> clauses = StreamSupport.stream(filterNode.get("conditions").spliterator(), false).map(c -> {
                if (c.has("conditions")) {
     ...
     */
    private static String buildSimpleCondition(JsonNode condition, Map<String, Object> requestParams,
            Map<String, Object> params) {
    ...
    private static void buildHaving(StringBuilder sql, JsonNode template, Map<String, Object> requestParams,
            Map<String, Object> params) {
    ...
    private static String buildHavingCondition(JsonNode condition, Map<String, Object> requestParams,
            Map<String, Object> params) {
        boolean isOptional = condition.path("optional").asBoolean(false);
    ...
    private static String extractAndBindParams(String expression, Map<String, Object> requestParams,
        ...
        boolean allParamsValid = paramNames.stream().allMatch(p -> {
        ...
        if (!allParamsValid) {
            if (isOptional) {
                ...
                Set<String> problematic = paramNames.stream().filter(p -> {
                    Object val = requestParams.get(p);
                    return val == null || (val instanceof String s && s.isBlank());
    ...
    private static void buildJoins(StringBuilder sql, JsonNode template) {
        Optional.ofNullable(template.get("joins")).filter(JsonNode::isArray).ifPresent(joins -> {
            for (JsonNode join : joins) {
                ...
                if (join.has("on")) {
                    sql.append(" ON ");
                    String on = StreamSupport.stream(join.get("on").spliterator(), false).map(o -> {
    ...
    private static Integer extractIntegerValue(JsonNode node, Map<String, Object> requestParams,
    ...
    private static void buildGroupBy(StringBuilder sql, JsonNode template) {
        Optional.ofNullable(template.get("groupBy")).filter(JsonNode::isArray).ifPresent(gb -> {
            String groups = StreamSupport.stream(gb.spliterator(), false).map(JsonNode::asText)
    ...
    private static String parseSortObject(String sortParam) {
        ...
        try {
            Map<String, String> sortMap = MAPPER.readValue(cleanParam,
                    new TypeReference<LinkedHashMap<String, String>>() {
                    });

            return sortMap.entrySet().stream().map(entry -> entry.getKey() + " " + normalizeDirection(entry.getValue()))
                    .collect(Collectors.joining(", "));
    ...
    private static void buildReturning(StringBuilder sql, JsonNode template) {
        Optional.ofNullable(template.get("returning")).filter(JsonNode::isArray).ifPresent(r -> {
            String fields = StreamSupport.stream(r.spliterator(), false).map(JsonNode::asText)
                    .collect(Collectors.joining(", "));
            if (!fields.isBlank())
                sql.append(" RETURNING ").append(fields);
        });
    }
    ...
    private static Set<String> parseFields(Object fieldsParam) {
        ...
        case String s when !s.isBlank() -> Arrays.stream(s.replaceAll("[\\[\\]]", "").split(",")).map(String::trim)
