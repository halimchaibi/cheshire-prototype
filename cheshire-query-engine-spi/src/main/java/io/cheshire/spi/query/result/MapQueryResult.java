...
/**
 * Map-based QueryResult implementation.
 * Each row is represented as Map<String,Object>.
 * Columns use a simple built-in column type.
 */
public final class MapQueryResult implements QueryResult<Map<String, Object>, MapQueryResult.Column> {
     ...
     *
     * <p>Represents metadata for a single column in a query result, including
     * the column name, data type, and nullability information.</p>
     *
     * @param name     the column name (case-sensitive as returned by the query)
     * @param type     the data type name (e.g., "VARCHAR", "INTEGER", "DOUBLE")
    ...
    public record Column(String name, String type, boolean nullable) {
         ...
         * @param type     the data type name, must not be null
