 ...
 * <p>
 * CheshireRuntime bridges the logical layer ({@link CheshireSession}) with the network
 * transport layer ({@link CheshireServer}), managing the complete lifecycle of protocol
 * exposures (REST, MCP, GraphQL, WebSockets).
 * <p>
 ...
 * All state transitions use atomic operations. The runtime uses:
 * <ul>
...
public final class CheshireRuntime implements AutoCloseable {
     ...
     * <p>
     * This is the primary factory method for creating runtime instances. It bridges
     * the business logic layer ({@link CheshireSession}) with the network transport layer.
     * <p>
     * <strong>Design Rationale:</strong>
     * The separation between Session and Runtime allows:
     * <ul>
     ...
     * </ol>
     * <p>
     * <strong>Concurrent Server Startup:</strong>
     * Uses {@link StructuredTaskScope} for fail-fast parallel initialization.
     * If any server fails to start, all servers are stopped and runtime transitions
     * to FAILED state.
     * <p>
     * <strong>Idempotency:</strong> Safe to call multiple times. Subsequent calls
     * after successful start are no-ops returning the existing instance.
     *
    ...
    public CheshireRuntime start() {
        ...
        try (var scope = new StructuredTaskScope.ShutdownOnFailure()) {
            ...
            session.capabilities().all().forEach(capability -> scope.fork(() -> {
                CheshireServer server = CheshireServerFactory
                        .load(capability.transport().getFactory())
                        .create(capability, capability.exposure().getBinding(), CheshireDispatcher.from(capability, session));
     ...
     * <p>
     * This method provides fine-grained control for scenarios where only one
     * capability should be exposed, such as:
     * <ul>
     ...
     * </ul>
     * <p>
     * <strong>Note:</strong> Unlike {@link #start()}, this method does not use
     * structured concurrency since only one server is created.
     *
     ...
     * <p>
     * Delegates to {@link #close()} for consistent shutdown behavior.
     * This method exists for API clarity and AutoCloseable compatibility.
     *
     * @see #close()
     */
    public void stop() {
     ...
     * If shutdown doesn't complete within 30 seconds, forces termination.
     * Shutdown hook failures are logged but don't prevent other hooks from running.
     * <p>
     * <strong>Thread Safety:</strong> Uses lock to ensure only one shutdown sequence
     * executes even if called from multiple threads.
     * <p>
     * <strong>Idempotency:</strong> Safe to call multiple times. Subsequent calls
     * after shutdown initiated are no-ops.
     */
    @Override
    public void close() {
     ...
     * </ul>
     * <p>
     * <strong>Interruption Handling:</strong>
     * If the calling thread is interrupted, the method returns immediately
     * and preserves the interrupt status.
     * <p>
     * <strong>Coordination Mechanism:</strong>
     * Uses a {@link Condition} variable that is signaled when the runtime
     ...
     * <p>
     * Returns {@code true} only when the runtime has successfully started
     * and has not yet begun shutdown.
     *
     ...
     * </ul>
     * <p>
     * <strong>Thread Safety:</strong> Callbacks execute on the thread that transitions
     * the runtime to RUNNING state. Multiple callbacks are supported.
     * <p>
     * <strong>One-Time Execution:</strong> Each callback executes exactly once and
    ...
    private enum State {NEW, STARTING, RUNNING, STOPPING, STOPPED, FAILED}

    public sealed interface RuntimeEvent permits CheshireRuntime.RuntimeStarted, CheshireRuntime.RuntimeStopped, CheshireRuntime.RuntimeError {
        Instant timestamp();
    }
