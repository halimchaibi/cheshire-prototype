 ...
 * <p>
 * Represents the physical HTTP server infrastructure ("The Engine"). Owns the ServerSocket, thread pool, and network
 * port. Multiple server handles (REST, MCP) can share a single container instance.
 * <p>
 ...
 * <p>
 * Tracks number of registered handles. Server starts when first handle registers, stops when last handle unregisters.
 * Thread-safe ref counting with atomic operations.
 * <p>
 * <strong>Virtual Thread Integration:</strong>
 * <p>
 * Configures Jetty's thread pool to use Virtual Threads for request handling, enabling massive concurrency with minimal
 * resource overhead.
 * <p>
 * <strong>CORS Handling:</strong>
 * <p>
 * TODO: **REVIEW CORS HANDLER** - Jetty 12 deprecated CORS as filter. Consider migrating to Server → CORS → Contexts →
 * Servlets pattern.
 * <p>
        ...
        if (content instanceof ServletContextHandler context) {
            ...
            if (isStarted.get()) {
                try {
                    ...
                    log.error("Failed to start hot-deployed context '{}' on container port {}", contextPath,
                            transportConfig.getPort(), e);
                    throw new RuntimeException("Failed to start hot-deployed context '%s' on container port %d"
                            .formatted(contextPath, transportConfig.getPort()), e);
                }
            }
