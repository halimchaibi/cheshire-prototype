 ...
 *
 * <p>Represents the logical service layer ("The Tenant"). Owns the capability-specific dispatcher
 * and API paths. Delegates physical server operations to the shared {@link JettyServerContainer}.
 *
 ...
 *
 * <ol>
 *   <li>{@link #init()} - Initialize configuration (no network)
 *   <li>{@link #start()} - Attach to container, register context, start server
 *   <li>{@link #stop()} - Reference-counted shutdown via container
 * </ol>
 *
 * <p><strong>Exposure Type Routing:</strong>
 *
 * <ul>
 *   <li><strong>REST_HTTP:</strong> → {@link JettyRestModule}
 *   <li><strong>MCP_STREAMABLE_HTTP:</strong> → {@link JettyMcpModule}
 * </ul>
 *
 * <p><strong>Multi-Tenancy:</strong>
 *
 * <p>Multiple handles can share a single container (port), each serving different capabilities at
 * different paths.
 *
 * @see CheshireServer
...
public final class JettyServerHandle implements CheshireServer {
   ...
   *
   * <p>This phase should include loading the {@code CheshireConfig}, initializing the {@code
   * ProtocolAdapter}, and setting up internal handlers. No network ports should be opened during
   * this phase.
   *
  ...
  public void start() throws Exception {
    ...
    ServletContextHandler handler =
        switch (ExposureType.from(capability.exposure().getType())) {
