 ...
 *
 * <p>Provides singleton transport instances keyed by port number, enabling multiple capabilities to
 * share a single HTTP server (Jetty instance) when configured with the same port.
 *
 * <p><strong>Transport Sharing Strategy:</strong>
 *
 * <ul>
 *   <li><strong>HTTP/Jetty:</strong> One container per unique port number (shareable)
 *   <li><strong>Stdio:</strong> Dedicated instance per capability (non-shareable)
 * </ul>
 *
 * <p><strong>Example Sharing:</strong>
 *
 * <pre>{@code
 * # Configuration:
 ...
 *
 * <p>Uses {@link ConcurrentHashMap#computeIfAbsent} for atomic get-or-create operations. Safe for
 * concurrent capability initialization.
 *
 * <p><strong>Lifecycle:</strong>
 *
 * <p>Containers are created lazily on first access and shared across capabilities. Reference
 * counting in {@link JettyServerContainer} manages actual shutdown.
 *
...
public class ServerRegistry {
   ...
   *
   * <p>Creates {@link JettyServerContainer} for network bindings (HTTP) keyed by port, or dedicated
   * {@link StdioServerContainer} for stdio bindings.
   *
   * @param capability capability defining transport configuration
  ...
  public static CheshireTransport getOrCreate(Capability capability) {
    ...
    return ENGINES.computeIfAbsent(
        port,
        p -> {
          CheshireTransport.Binding binding =
              CheshireTransport.Binding.from(capability.exposure().getBinding());
