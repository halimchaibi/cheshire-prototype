...
public class RestProtocolAdapter
    implements ProtocolAdapter<HttpServletRequest, Map<String, Object>> {
  ...
  public RequestEnvelope toRequestEnvelope(HttpServletRequest req, String capability)
    ...
    try {
      ...
      req.getHeaderNames()
          .asIterator()
          .forEachRemaining(name -> headers.put(name, req.getHeader(name)));

      ProtocolMetadata protocolMeta =
          new ProtocolMetadata("REST", "1.0", headers, req.getRequestURI(), req.getMethod());
      ...
      RequestPayload payload =
          new RequestPayload(
              determinePayloadType(req.getContentType()),
              parseBody(req), // The
              // raw
              // data
              // object
    ...
    return (T)
        switch (result) {
          case ResponseEntity.Success(Object data, var meta) -> {
            ...
            switch (data) {
              case Map<?, ?> dataMap ->
                  dataMap.forEach((k, v) -> response.put(String.valueOf(k), v));
  ...
  private RequestPayload.PayloadType determinePayloadType(String contentType) {
    if (contentType == null) return RequestPayload.PayloadType.EMPTY;
    if (contentType.contains("json")) return RequestPayload.PayloadType.JSON;
    if (contentType.contains("xml")) return RequestPayload.PayloadType.XML;
