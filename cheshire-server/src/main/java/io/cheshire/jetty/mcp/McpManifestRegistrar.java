...
@Slf4j
public record McpManifestRegistrar(Capability capability, CheshireDispatcher dispatcher, McpAsyncServer server,
    ...
    public static McpManifestRegistrar of(Capability capability, CheshireDispatcher dispatcher, McpAsyncServer server,
            McpProtocolAdapter protocolAdapter) {
        return new McpManifestRegistrar(capability, dispatcher, server, protocolAdapter);
    }
    ...
    public void register() {
        ...
        log.info("Capability '{}' registered with {} tools, {} resources, {} resource templates, and {} prompts",
                capability.name(), capability.actions().tools.size(), capability.actions().resources.size(),
                capability.actions().resourceTemplates.size(), capability.actions().prompts.size());
    }
    ...
    private void registerTools(List<ActionsConfig.Tool> tools, McpAsyncServer server) {
        for (ActionsConfig.Tool tool : tools) {
            try {
                ...
                server.addTool(toolSpec).doOnSuccess(v -> log.info("Successfully bound tool: {}", tool.name)).doOnError(
                        e -> log.error("Failed to bind tool '{}' for capability '{}'", tool.name, capability.name(), e))
                        .subscribe(); // Welcome to the hell, with reactive programming. Nothing will happen until we
                                      // subscribe; the addTool returns a Mono.
                log.debug("Registered tool: {}", tool.name);
            } catch (Exception e) {
                log.error("Failed to register tool '{}' for capability '{}' during specification creation", tool.name,
                        capability.name(), e);
            }
        }
    }
    ...
    private McpServerFeatures.AsyncToolSpecification createToolSpecification(ActionsConfig.Tool tool) {
        McpSchema.Tool mcpTool = McpSchema.Tool.builder().name(tool.name).description(tool.description)
                .inputSchema(SchemaConverter.convertToJsonSchema(tool.inputSchema)).meta(tool.metadata).build();

        return new McpServerFeatures.AsyncToolSpecification.Builder().tool(mcpTool).callHandler((exchange, request) -> {
            return Mono
                    .fromCallable(
                            () -> dispatcher.dispatch(protocolAdapter.toRequestEnvelope(request, capability.name())))
                    .subscribeOn(Schedulers.boundedElastic()).handle((response, sink) -> {
                        try {
                            ...
                            sink.error(new RuntimeException(
                                    "Protocol mapping failed for tool request '%s' in capability '%s': %s"
                                            .formatted(request.name(), capability.name(), e.getMessage()),
                                    e));
                        }
                    });
        }).build();
        ...
        for (ActionsConfig.Resource resource : resources) {
            try {
                ...
                server.addResource(resourceSpec)
                        ...
                        .doOnError(e -> log.error("Failed to bind resource '{}' for capability '{}'", resource.name,
                                capability.name(), e))
                        .subscribe();
                log.debug("Registered resource: {}", resource.name);
            } catch (Exception e) {
                log.error("Failed to register resource '{}' for capability '{}' during specification creation",
                        resource.name, capability.name(), e);
            }
        }
    }

    private McpServerFeatures.AsyncResourceSpecification createResourceSpecification(ActionsConfig.Resource resource) {
        McpSchema.Resource mcpResource = McpSchema.Resource.builder().name(resource.name).uri(resource.uri)
                .description(resource.description).mimeType(resource.mimeType).build();

        return new McpServerFeatures.AsyncResourceSpecification(mcpResource, (exchange, request) -> Mono
                .fromCallable(() -> dispatcher.dispatch(protocolAdapter.toRequestEnvelope(request, capability.name())))
                .subscribeOn(Schedulers.boundedElastic()).handle((response, sink) -> {
                    try {
                        ...
                        sink.error(new RuntimeException(
                                "Protocol mapping failed for resource request '%s' in capability '%s': %s"
                                        .formatted(request.uri(), capability.name(), e.getMessage()),
                                e));
                    }
                })
    ...
    private void registerPrompts(List<ActionsConfig.Prompt> prompts, McpAsyncServer server) {
        for (ActionsConfig.Prompt prompt : prompts) {
            try {
                ...
                server.addPrompt(promptSpec).doOnSuccess(v -> log.info("Successfully bound prompt: {}", prompt.name))
                        .doOnError(e -> log.error("Failed to bind prompt '{}' for capability '{}'", prompt.name,
                                capability.name(), e))
                        .subscribe();
            } catch (Exception e) {
                log.error("Failed to register prompt '{}' for capability '{}' during specification creation",
                        prompt.name, capability.name(), e);
            }
        }
    }
    ...
    private McpServerFeatures.AsyncPromptSpecification createPromptSpecification(ActionsConfig.Prompt prompt) {
        List<McpSchema.PromptArgument> mcpArgs = (prompt.arguments != null)
                ? prompt.arguments.stream()
                        .map(arg -> new McpSchema.PromptArgument(arg.name, arg.description, arg.required)).toList()
                : new ArrayList<>();

        McpSchema.Prompt mcpPrompt = new McpSchema.Prompt(prompt.name, null, // title
                prompt.description, mcpArgs);

        return new McpServerFeatures.AsyncPromptSpecification(mcpPrompt, (exchange, request) -> Mono
                .fromCallable(() -> dispatcher.dispatch(protocolAdapter.toRequestEnvelope(request, capability.name())))
                .subscribeOn(Schedulers.boundedElastic()).handle((response, sink) -> {
                    try {
                        ...
                        sink.error(new RuntimeException(
                                "Protocol mapping failed for prompt request '%s' in capability '%s': %s"
                                        .formatted(request.name(), capability.name(), e.getMessage()),
                                e));
                    }
                }));
    ...
    private McpServerFeatures.AsyncResourceTemplateSpecification createResourceTemplateSpecification(
            ActionsConfig.ResourceTemplate template) {
        McpSchema.ResourceTemplate mcpTemplate = McpSchema.ResourceTemplate.builder().uriTemplate(template.uriTemplate)
                .name(template.name).description(template.description).mimeType(template.mimeType)
                .meta(template.metadata) // Assuming your metadata maps to the internal meta Map
                .build();
        return new McpServerFeatures.AsyncResourceTemplateSpecification(mcpTemplate,
                (exchange, request) -> Mono.fromCallable(() -> {
                    return dispatcher.dispatch(protocolAdapter.toRequestEnvelope(request, capability.name()));
                }).subscribeOn(Schedulers.boundedElastic()).handle((response, sink) -> {
                    try {
