...
// TODO: Refactor to split the tool / resource / prompt registration into separate classes to reduce
// complexity
@Slf4j
public record McpManifestRegistrar(
  ...
  public void register() {
    ...
    final List<ActionsConfig.Tool> tools = entries(capability.actions().getTools());
    final List<ActionsConfig.Resource> resources = entries(capability.actions().getResources());
    final List<ActionsConfig.Prompt> prompts = entries(capability.actions().getPrompts());
    final List<ActionsConfig.ResourceTemplate> resourceTemplates =
        entries(capability.actions().getResourceTemplates());

    registerTools(tools, server);
    registerResources(resources, server);
    registerPrompts(prompts, server);
    registerResourceTemplates(resourceTemplates, server);

    log.info(
        "Capability '{}' registered with {} tools, {} resources, {} resource templates, and {} prompts",
        capability.name(),
        tools.size(),
        resources.size(),
  ...
  private static <T> List<T> entries(final List<T> entries) {
    return Optional.ofNullable(entries).orElseGet(List::of);
  }
      ...
      try {
        ...
        server
            ...
            .doOnError(
                e ->
                    log.error(
                        "Failed to bind tool '{}' for capability '{}'",
                        tool.name,
                        capability.name(),
                        e))
            .subscribe();
        ...
        log.error(
            "Failed to register tool '{}' for capability '{}' during specification creation",
            tool.name,
            capability.name(),
            e);
      }
                      ...
                      (response, sink) -> {
                        try {
                          McpSchema.CallToolResult result =
                              protocolAdapter.fromProcessingResult(request, response);
    ...
    for (ActionsConfig.Resource resource : resources) {
      try {
        McpServerFeatures.AsyncResourceSpecification resourceSpec =
            createResourceSpecification(resource);
        server
            ...
            .doOnError(
                e ->
                    log.error(
                        "Failed to bind resource '{}' for capability '{}'",
                        resource.name,
                        capability.name(),
                        e))
            .subscribe();
        ...
        log.error(
            "Failed to register resource '{}' for capability '{}' during specification creation",
            resource.name,
            capability.name(),
            e);
      }
  ...
  private McpServerFeatures.AsyncResourceSpecification createResourceSpecification(
      ActionsConfig.Resource resource) {
    McpSchema.Resource mcpResource =
  ...
  private void registerPrompts(List<ActionsConfig.Prompt> prompts, McpAsyncServer server) {
    for (ActionsConfig.Prompt prompt : prompts) {
      try {
        ...
        log.error(
            "Failed to register prompt '{}' for capability '{}' during specification creation",
            prompt.name,
            capability.name(),
            e);
      }
  ...
  private McpServerFeatures.AsyncPromptSpecification createPromptSpecification(
      ActionsConfig.Prompt prompt) {
    final List<McpSchema.PromptArgument> mcpArgs =
        entries(prompt.arguments).stream()
            .map(arg -> new McpSchema.PromptArgument(arg.name, arg.description, arg.required))
            .toList();

    McpSchema.Prompt mcpPrompt =
  ...
  private void registerResourceTemplates(
    ...
    for (ActionsConfig.ResourceTemplate template : templates) {
      try {
        ...
        log.error(
            "Failed to register resource template '{}' during specification creation",
            template.name,
            e);
      }
    }
  ...
  private McpServerFeatures.AsyncResourceTemplateSpecification createResourceTemplateSpecification(
    ...
    McpSchema.ResourceTemplate mcpTemplate =
