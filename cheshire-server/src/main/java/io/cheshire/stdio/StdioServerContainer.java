 ...
 *
 * <p>Manages MCP server instances communicating via standard input/output streams. Unlike HTTP
 * transports, stdio transport supports only a single registered module.
 *
 * <p><strong>Protocol:</strong>
 *
 * <p>Implements Model Context Protocol (MCP) over stdio:
 *
 * <ul>
 *   <li>Reads JSON-RPC messages from stdin
 *   <li>Writes JSON-RPC responses to stdout
 *   <li>No network ports or HTTP overhead
 *   <li>Suitable for CLI tools, subprocess communication
 * </ul>
 *
 * <p><strong>Single Module Constraint:</strong>
 *
 * <p>Only one {@link McpAsyncServer} can be registered per stdio transport since there's only one
 * stdin/stdout pair. Attempting to register multiple modules throws {@link IllegalStateException}.
 *
 * <p><strong>Lifecycle:</strong>
 *
 * <ul>
 *   <li>{@link #register(Object)} - Register MCP server (once)
 *   <li>{@link #start()} - Start stdio communication
 *   <li>{@link #stop()} - Stop stdio communication
 * </ul>
 *
