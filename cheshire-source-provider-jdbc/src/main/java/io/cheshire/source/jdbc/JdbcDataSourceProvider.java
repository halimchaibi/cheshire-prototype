 ...
 * <p>
 * Provides access to relational databases via JDBC, handling connection pooling, query execution, parameter binding,
 * and result set extraction.
 * <p>
...
public final class JdbcDataSourceProvider implements SourceProvider<SqlQuery, SqlQueryResult> {
    ...
    private void bindParameters(PreparedStatement pstmt, Map<String, List<Integer>> paramMap,
    ...
    private HikariDataSource createDataSource() throws SourceProviderException {
        ...
        hikariConfig.setPoolName(getFromPoolConfig("poolName", config.name() + "-pool", Object::toString));
    ...
    public void open() throws SourceProviderException {
        ...
        try {
            ...
            log.info("Pool stats - Max: {}, MinIdle: {}", dataSource.getMaximumPoolSize(), dataSource.getMinimumIdle());
        } catch (Exception e) {
            if (dataSource != null) {
                dataSource.close();
                dataSource = null;
    ...
    /**
     * Executes a batch operation using a single connection from the pool. This is more efficient than executing queries
     * individually.
     *
     * @param query
     *            The SQL query template
     * @param batchParams
     *            List of parameter maps for each batch execution
     * @return Result containing total affected rows and batch results
     * @throws SourceProviderException
     *             if execution fails
     */
    public SqlQueryResult executeBatch(SqlQuery query, List<Map<String, Object>> batchParams)
            throws SourceProviderException {
        ensureOpen();
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            return SqlQueryResult.of(List.of(Map.of("totalAffected", totalAffected, "batchResults", results)));
        } catch (SQLException e) {
    ...
    public Map<String, Object> getPoolStats() {
        ...
        try {
            return Map.of("active", dataSource.getHikariPoolMXBean().getActiveConnections(), "idle",
                    dataSource.getHikariPoolMXBean().getIdleConnections(), "total",
                    dataSource.getHikariPoolMXBean().getTotalConnections(), "waiting",
                    dataSource.getHikariPoolMXBean().getThreadsAwaitingConnection(), "maxPoolSize",
                    dataSource.getMaximumPoolSize(), "minIdle", dataSource.getMinimumIdle());
        } catch (Exception e) {
