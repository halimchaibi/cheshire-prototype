 ...
 *
 * <p>
 * This provider supports thread-safe operations for both data retrieval and modification.
 * <p>
 * Key features:
 *
 * <ul>
 ...
 *
 * <p>
 * <strong>Thread Safety:</strong> This implementation is thread-safe. Multiple threads
 * <p>
 * can safely execute queries concurrently. The {@link #open()} and {@link #close()} methods
 * <p>
 * are synchronized to ensure proper manager management.
 * </p>
 *
 *
 *
 ...
 *
 * <p>
 * <strong>Named Parameters:</strong> Supports named parameters in SQL queries using
 * <p>
...
public final class JdbcDataSourceProviderBack implements SourceProvider<SqlQuery, SqlQueryResult> {
     ...
     *
     * @param config
     *            the JDBC data source configuration, must not be null
        ...
        try {
            ...
            try (Connection conn = getConnection()) {
                ...
                if (conn.isValid(5)) {
                    ...
                    log.info("JdbcDataSourceProviderBack '{}' opened successfully in {}ms (url: {})", config.name(),
                            duration, config.require("jdbcUrl"));

                } else {

                    throw new SourceProviderException(
                            String.format("Connection validation failed for source '%s'", config.name()));
            ...
            String errorMsg = String.format("JDBC driver class not found for source '%s': %s", config.name(),
                    e.getMessage());
            ...
            String errorMsg = String.format("Failed to establish connection for source '%s' (url: %s): %s",
                    config.name(), config.require("jdbcUrl"), e.getMessage());
            ...
            String errorMsg = String.format("Unexpected error opening JdbcDataSourceProviderBack '%s': %s",
     ...
     *
     * <p>
     * Automatically distinguishes between SELECT queries (returns rows) and
     * <p>
     * UPDATE/INSERT/DELETE statements (returns update count).
     * </p>
     *
     * @param query
     ...
     *             if execution fails or provider is not open
     */
    ...
    private List<Map<String, Object>> query(String sql, Map<String, Object> params) throws SourceProviderException {
        ...
        try (Connection conn = getConnection(); PreparedStatement pstmt = conn.prepareStatement(parsed.sql)) {
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                ...
                log.debug("SELECT query executed on source '{}' in {}ms, returned {} rows", config.name(), duration,
                        results.size());
            ...
            String errorMsg = String.format("Failed to execute SELECT query on source '%s' for query '%s': %s",
                    config.name(), sql, e.getMessage());
    ...
    private int executeUpdate(String sql, Map<String, Object> params) throws SourceProviderException {
        ...
        try (Connection conn = getConnection(); PreparedStatement pstmt = conn.prepareStatement(parsed.sql)) {
            ...
            log.debug("DML/DDL statement executed on source '{}' in {}ms, affected {} rows", config.name(), duration,
                    updateCount);
            ...
            String errorMsg = String.format("Failed to execute DML/DDL statement on source '%s' for statement '%s': %s",
                    config.name(), sql, e.getMessage());
    ...
    private void ensureOpen() throws SourceProviderException {
        ...
        if (!open) {
            ...
            try {
                ...
                throw new SourceProviderException(String.format("Failed to open JdbcDataSourceProviderBack '%s': %s",
    ...
    private void bindParameters(PreparedStatement pstmt, ParsedQuery parsed, Map<String, Object> params)
            throws SQLException {
     ...
     *
     * <p>
     * After parsing, named parameters (e.g., :paramName) are replaced with
     * <p>
     * positional placeholders (?) and a mapping is created from parameter names
     * <p>
     * to their positions in the prepared statement.
     * </p>
     *
     * @param sql
     *            the SQL with named parameters replaced by ?
     * @param paramMap
     *            mapping from parameter names to their positions in the SQL
     */
    ...
    private record ParsedQuery(String sql, Map<String, List<Integer>> paramMap) {
         ...
         *
         * @param sql
         *            the SQL with positional parameters
         * @param paramMap
         *            the parameter name to position mapping
         */
