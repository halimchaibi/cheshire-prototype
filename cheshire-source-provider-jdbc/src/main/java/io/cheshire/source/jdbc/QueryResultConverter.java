 ...
 * <p>
 * Maps Java types to string representations: Integer, Long, Float, Double,
 * Boolean, String, Date. Falls back to simple class name for other types.
 * <p>
...
public final class QueryResultConverter {
    ...
    /**
     * Converts a list of rows (Map<String,Object>) into a MapQueryResult.
     * Column metadata is inferred from the first row.
     *
     * @param rows list of result rows from JDBC
    ...
    public static MapQueryResult fromRows(List<Map<String, Object>> rows) {
        ...
        List<MapQueryResult.Column> columns = firstRow.keySet().stream()
                .map(key -> new MapQueryResult.Column(
                        key,
                        inferType(firstRow.get(key)),
                        true // assuming all columns nullable by default
                ))
