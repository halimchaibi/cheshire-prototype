...
public class CodingStandardsTest {
     ...
     * </ul>
     * <p>
     * <strong>Exception:</strong> Some framework internals may require mutability
     * for performance (e.g., AtomicReference for state machines).
     */
    @ArchTest
    static final ArchRule fields_Should_Be_Final_For_Immutability =
            fields().that().areDeclaredInClassesThat()
                    ...
                    .and().arePrivate()
                    .should().beFinal()
                    .orShould().haveRawType("java.util.concurrent.atomic.AtomicReference")
                    .orShould().haveRawType("java.util.concurrent.atomic.AtomicBoolean")
                    .orShould().haveRawType("java.util.concurrent.atomic.AtomicInteger")
                    .orShould().haveRawType("java.util.concurrent.atomic.AtomicLong")
    ...
    static final ArchRule value_Objects_Should_Be_Records =
            classes().that().resideInAPackage("io.cheshire..")
                    .and().haveSimpleNameEndingWith("DTO")
                    .or().haveSimpleNameEndingWith("Value")
    ...
    static final ArchRule finder_Methods_Should_Return_Optional =
            methods().that().haveName("find")
                    .or().haveNameStartingWith("find")
                    .or().haveNameStartingWith("get")
                    .and().areDeclaredInClassesThat().resideInAPackage("io.cheshire..")
                    .and().arePublic()
                    .should().haveRawReturnType("java.util.Optional")
                    .orShould().haveRawReturnType("java.util.List")
                    .orShould().haveRawReturnType("java.util.Set")
                    .orShould().haveRawReturnType("java.util.Map")
    ...
    static final ArchRule exceptions_Should_Not_Be_Generic =
            methods().that().areDeclaredInClassesThat()
                    ...
                    .orShould().notDeclareThrowableOfType(Throwable.class)
     ...
     * <p>
     * <strong>Java 21 Feature:</strong> Records provide concise syntax for immutable
     * data carriers with built-in equals/hashCode/toString.
     */
    @ArchTest
     ...
     * </ul>
     * <p>
     * <strong>Note:</strong> Cheshire doesn't use a DI framework, but the principle
     * of constructor injection still applies for dependency management.
     */
    @ArchTest
