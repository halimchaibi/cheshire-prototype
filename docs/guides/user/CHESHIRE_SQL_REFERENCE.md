...
### Arrow Source Mapping

For Arrow-backed federation, the source name becomes the SQL schema and the inner `directory`
value points to a local directory containing Arrow Feather files. Each `.arrow` file is exposed
as a table using the file name without the suffix.
...
Query Arrow tables the same way as other SQL-facing sources:

```sql
SELECT fieldOne, fieldTwo, fieldThree
FROM warehouse.events
WHERE fieldOne >= ?
ORDER BY fieldOne ASC
FETCH NEXT 100 ROWS ONLY
```

### Additional Calcite Source Mappings

The Calcite engine also registers these read-oriented source types. In every case, the
configured source name becomes the SQL schema name used in queries.

| Source type | Required configuration | Notes |
|-------------|------------------------|-------|
| `csv` | `directory` | Exposes local `.csv` files as tables through Calcite file schema support. |
| `file` | `directory` or `tables` | Discovers local `.csv` and `.json` files, or registers explicit table URLs. |
| `cassandra` | `host`, `keyspace` | Optional `port`, `username`, and `password`; schema creation opens a Cassandra session. |
| `innodb` | `sqlFilePath`, `ibdDataFileBasePath` | Reads InnoDB table metadata from SQL DDL files and data files from the base path. |
| `os` | none | Exposes OS and JVM metadata tables such as `system_info`, `java_info`, and `os_version`. |
| `redis` | `host`, `port`, `database`, `tables` | Each table declares `dataFormat` and `fields` in its operand. |
| `kafka` | `tables` | Each table declares Kafka consumer operands such as `bootstrap.servers` and `topic.name`. |
| `mat` | `factoryClass` | Plugin hook for MAT-compatible Calcite schema factories available on the classpath. |
...
Example file source with an explicit table:
...
sources:
  reports:
    ...
    config:
      ...
      tables:
        - name: departments
          url: file:///opt/cheshire/reports/departments.html
```
...
Query these sources by qualifying the table with the source schema:
